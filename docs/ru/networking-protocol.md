...
- **10 секунд** на завершение handshake;
- отсутствие обычного post-handshake idle timeout (`Timeout.InfiniteTimeSpan`);
- connection rate budget `HardAbuse`;
- набор per-message rate limits `HardAbuse`.

Успешный handshake обновляет last-inbound timestamp и будит watchdog, чтобы смена режима policy наблюдалась немедленно.
...
| `HandshakeTimeout` | handshake не завершён до deadline |
| `IdleTimeout` | истёк настроенный post-handshake inactivity deadline |
...
| `RateLimited` | connection/message budget отклонил трафик |

Эти причины специально не объединяются в один «network error». Malformed input, abusive rate, неподдерживаемая версия клиента и I/O failure требуют разной диагностики.
...
В roadmap всё ещё остаются более широкие work-budget задачи, включая полные subsystem-level budgets для дорогих операций. Поэтому текущие connection/message limits нельзя описывать как завершённую DoS-защиту всех gameplay подсистем.

## 10. Authoritative command queue

Decoded network input попадает в simulation через bounded command path. Game loop применяет global и per-source fairness limits, чтобы одно соединение не могло монополизировать tick, постоянно удерживая очередь непустой.
...
Malformed или abusive client traffic должен закрывать/отклонять конкретное соединение, а не падать всем server process и не ломать shutdown save path.
...
- permanent protocol fuzz corpus для всех variable-length decoders;
- полные global/per-subsystem expensive-work budgets;
