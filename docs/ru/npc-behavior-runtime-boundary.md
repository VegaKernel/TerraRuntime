...
## Порядок обхода живых слотов

Исполнитель NPC обходит физические слоты по возрастанию и читает актуальное состояние непосредственно перед каждым шагом AI, как `Main.Update` / `NPC.UpdateNPC` в TerrariaServer `1.4.5.8`. Перемещения, удаления и замены соседних NPC видны последующим слотам. Новый NPC в более высоком слоте обновляется в текущем тике; в уже пройденном слоте — в следующем. Замена NPC во время подготовки его собственного перехода по-прежнему отклоняет этот переход и запланированные эффекты по точному поколению.

Перед каждым шагом потребители состояний соседних NPC получают свежий ограниченный снимок. Буферы переиспользуются, но за тик копируется не более `Capacity` в квадрате снимков (`Capacity <= 256`); увеличение допустимой таблицы требует замены этой границы копирования авторитетным интерфейсом чтения живого состояния. Это устанавливает порядок обхода NPC, но не доказывает полный паритет AI или порядка обхода снарядов.

Подтверждённые эффекты NPC могут использовать фазу после выделения частей и привязки слотов; перед её вызовом исполнитель повторно проверяет поколение и ревизию источника. Перемещение с проверкой точного поколения публикует `NpcStateCommitKind.ForcedUpdate`. `RuntimeNpcStore.TryUpdate` также принимает явный флаг `forceSync` для такой обязательной синхронизации. Реестр репликации обходит обычный интервал синхронизации движения, сохраняя формат пакета, актуальные baseline и телеметрию. Эта граница поддерживает оригинальный порядок перемещения ядра и частей Moon Lord.
...
Runtime-owned boss boundary теперь содержит оставшийся server-authoritative NPC-side state позднего Hardmode/endgame roster вместо encounter root, привязанных к metadata-only заглушкам projectile. Duke Fishron допускает AI 71 Sharkron/Sharkron2 с emergence/charge state и source-owned Sharknado Bolt `385`. Sharkron2 продвигает исходную косинусную emergence-wave как смещение позиции, сохраняет принадлежащие AI alpha/rotation и collision-retirement transitions и отклоняет неизвестные состояния AI71 до мутации; projectile runtime исполняет aiStyle-65 movement и generation-safe on-kill переход в Sharknado `384` либо terrain-anchored Cthulunado `386`. Lunatic Cultist допускает Ancient Vision/Light/Doom, ritual spawn Dragon либо Vision, прерывание ритуала попаданием по настоящему боссу или копии и source-owned семейства атак `464/465/467/468/490/593`.

Empress of Light staging-ит специализированные lasting-rainbow, rainbow-streak, lance и sun-dance projectile (`872/873/919/923`); дневная ярость также переводит cadence в source Expert-like режим и даёт projectile damage `9999`, а не полагается только на сохранённый `ai[3]`. В Remix-мире ярость вместо этого закрепляется, когда первая физическая slot Empress строго поднимается выше world surface, и не сбрасывается после возвращения вниз. Авторитетные часы мира также питают AI-120: действительно разъярённая Empress переходит к исходному уходу ночью либо на дневном тике `53 400`; state 13 меняет alpha на исходные пять единиц за тик, возвращаясь к атаке после 20 безопасных тиков или завершаясь при alpha 255. При половине здоровья state 1 запрашивает state 10, и только её тик 90 повышает phase через `ai[3]`, переносит boss над player и поднимает defense на 20 процентов. Dash states 8/9 уязвимы лишь на тиках 6–40 и используют 150 процентов contact damage на тиках 41–90. Attack clock рук, головы и True Eye Moon Lord следует закреплённым source sequence на 600/1200 тиков и staging-ит Phantasmal eye/sphere/deathray/leech/bolt intents (`452/454/455/456/462`). Moon Lord Hand также владеет закреплённым cross-entity release на tick 292: после commit точной generation руки только её generation-owned projectile type 454 получают `ai[0] = -1` и source speed 12. Первый смертельный удар по руке или голове теперь следует закреплённому переходу `NPC.checkDead`: life восстанавливается, часть становится неуязвимой в `ai[0] = -2`, из неё создаётся ровно один True Eye с исходным расчётом фазы по циклу 1200 тиков, базовому offset 588 и шагу 400 на каждый уже активный глаз, а core открывается только пока обе owned-руки и owned-голова всё ещё существуют в retired-state. Retired-голова также переходит в `ai[0] = -3`, когда core начинает death drama. Terminal death core проходит через авторитетный 600-tick lifecycle, а защищённый core напрямую удаляет себя, если точный сохранённый child-slot отсутствует, неактивен или содержит неверный тип. Cosmetic sound/dust/gore остаются вне authority. Для Moon Lord открыты глобальное event/announcement behavior, внутренности специальных projectile style и более широкие encounter differentials; оставшиеся seed-only условия Empress тоже остаются явной parity-работой.
