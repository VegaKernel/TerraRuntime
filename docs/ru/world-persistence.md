...
### 8.1 Объекты, которые жидкость уничтожает при загрузке мира

Мир, который сгенерировался начисто, всё равно может содержать объект, стоящий в лаве: генерация ставит объекты и
заливает пещеры разными проходами. Исходник разрешает это при загрузке: `WorldGen.WaterCheck` вызывает обычный
`KillTile` для одной найденной клетки, а собственный валидатор объекта убирает остальное через следующий за этим
`SquareTileFrame`. Исчезают ровно клетки самого объекта.

Какой прямоугольник теряет каждая идентичность — измерено, а не перечислено вручную. Зонд `.cache/watercheck-probe`
ставит по одному объекту каждой идентичности с прямоугольным следом `TileObjectData` внутри зафиксированной
официальной сборки, прогоняет настоящий проход сухим — чтобы доказать, что объект выживает сам по себе, — затем
залитым, и сравнивает весь мир. Каждый случай смерти очищает клетки самого объекта и не меняет идентичность ни
одной другой клетки, и ни один не бросает исключение. `tools/ci/generate_loading_object_footprints.py` превращает
этот свип в `VanillaLoadingObjectFootprint1458`: 237 идентичностей с шириной, высотой и шагами кадра, которыми
разложены их стили.

Шаги — причина, по которой таблица не сводится к ширине и высоте. Стиль не всегда сдвигает кадр на собственный
размер объекта: стул шириной в одну клетку и высотой в две, но его стили шагают на 40 пикселей вниз, а верстак
шириной в две и высотой в одну, но его стили шагают на 20. Место клетки внутри объекта — это её кадр по модулю
шага, а остаток, вышедший за пределы прямоугольника объекта, — это кадр, которого не даёт ни один стиль, и он
отвергается.

Три вещи остаются отвергнутыми. Идентичность, чей след меняется со стилем, потому что ни один прямоугольник её не
описывает — растительный детрит загрузчик по-прежнему называет сам, читая строку кадра, чтобы отличить
трёхширинную форму от двухширинной. Идентичность, несущая сундук, табличку или тайл-сущность, потому что убрать
её — значит убрать второй объект, над которым у этого пути нет полномочий. И несогласованный след: каждая клетка
обязана нести ровно тот кадр, который подразумевает выведенное начало, иначе не трогается ничего.

Один отказ был снят, а не сохранён. Платформа, чья опора снизу фреймозависима, раньше отвергалась — из теории, что
может последовать каскад. Не последует: то, что стоит ПОД платформой, никогда от неё не зависит; измерено на камне,
гранитной колонне, другой платформе, сундуке, столе и книжном шкафу, каждый построен по собственному следу. Во всех
шести исходник убирает платформу и оставляет опору ровно такой, какой нашёл. Потерять опору может только то, что
стоит НА платформе, и этот случай не изменился.

Результат: шестнадцать миров Large — восемь сидов на обе порчи — теперь генерируются и запускаются. До таблицы
примерно два из каждых девяти отказывались загружаться с `UnsupportedLiquidDeathTile` на идентичностях, до которых
рукописный список просто не был расширен.
...
При startup host выполняет тот же lease-safe cleanup для canonical world target и checkpoint-backup target ещё до cache/stat/load processing. Поэтому abandoned managed transaction убирается даже если после crash новый save ещё не происходил. Live lease по-прежнему защищает active writer, а legacy temporary без lease остаётся untouched. `Authoritative World Save` run `33270924996` независимо прогнал `AtomicSaveFileWriterCleanupTests` (`5/5`) и реальный startup-order test через `TerrariaServerHost.RunAsync` (`1/1`) перед остальным save pipeline.
...
`RuntimeWorldProgressionMutations` сохраняет загруженное исходное значение `LunarApocalypseIsUp`. Метод `SetLunarApocalypseIsUp` на авторитетном потоке записывает явные изменения true/false; снятые снимки не меняются при последующих обновлениях. Патчер сохранения изменяет только байт события при наличии явного изменения, сохраняя все четыре флага активности башен. Уход Лунного лорда сбрасывает событие без регистрации победы. Используется существующий путь checkpoint, версия формата кеша не меняется.

Владение мировым сундуком освобождается на авторитетном потоке при удалении игрока из мира, до завершения транзакции перехода. Отключение живого соединения использует ту же транзакцию. Новая сессия может открыть сундук; запоздалое отключение старой сессии не снимает владение новой. Это соответствует официальному `RemoteClient.Reset`, который заменяет ушедшего игрока новым `Player` с индексом сундука `-1`. Живая проверка сундуков включает обрыв соединения и повторный вход, восстановление предмета и имени, а также рассылку наблюдателю.

Если сгенерированный мир не оставил подходящего ровного пола возле спавна, тестовая табличка получает две каменные опоры в пустых сухих клетках. Существующие активные объекты сохраняются; физическая табличка размещается вместе с текстовой записью.
