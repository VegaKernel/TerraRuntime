...
Liquid contact TerraRuntime определяет самостоятельно по authoritative world tiles; host по-прежнему не передаёт тип жидкости или velocity. Проверенный ordinary unmounted path теперь сохраняет vanilla-порядок previous/current contact: contact прошлого tick выбирает gravity, фактический terminal fall speed и jump profile, а текущий contact выбирает collision displacement и становится profile state следующего tick. Фактические max-fall clamps: $10.01$ dry, $5.01$ в ordinary water/lava и $3.01$ в honey; shimmer contact сохраняет $10.01$. Jump speed/height: $5.01/15$ dry, $6.01/30$ water/lava, $5.01/15$ honey и $5.51/23$ shimmer. Current-contact displacement factors остаются $0.5$ для water/lava, $0.25$ для honey и $0.375$ для shimmer. Floating equipment/flippers, merman/trident movement, mounts, grapples, wings/extra jumps, shimmer transformation, drowning и lava damage остаются отдельной работой. Полный execution graph и transition rules описаны в [Авторитетной физике игрока](player-physics.md).
