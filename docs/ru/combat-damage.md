...
Обычное вычитание защиты NPC и умножение критического удара используют исходную точность `double`. Это сохраняет целый урон и половину защиты при значениях выше `2^24` до преобразования в урон здоровью; существующее ограничение `int.MaxValue` остаётся явной защитой от переполнения. Ещё 960 ударов оригинала проверяют большие входные значения публичного API вплоть до `100000000`, включая снижение RedHat после целочисленного округления. Предыдущий расчёт через float нарушает 450 таких сравнений. Эти значения проверяют авторитетный API, а не более узкий диапазон урона сетевого пакета.

Общий исполнитель урона NPC теперь применяет снижение RedHat Скелетрона после округления обычного результата защиты и критического удара: `max(1, (int)(damage * 0.7f))`. Он читает текущий авторитетный маркер AI/local-AI, а не определяет вариант по сложности мира. Уменьшенный урон используется для жизни и отбрасывания; исходный урон запроса и его происхождение сохраняются в результате. Независимые данные включают 1 760 ударов оригинала: нулевой/малый урон, отрицательную/нечётную/большую защиту, критические удары, неверное поле маркера и NPC без этого варианта. Другие модификаторы урона оригинала учитываются отдельно.
