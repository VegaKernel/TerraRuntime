...
World motion уже владеет tile collision, liquid contact, world bounds и source-backed lifetime. Server-owned hostile simulation slice теперь дополнительно исполняет source-backed прямые AI_001 beams Wall of Flesh, Probe, Retinazer и Golem (83/84/100/259), Plantera Seed/Poison Seed (275/276), включая Expert-mode homing и lifetime/tile-collision overrides, Plantera Thorn Ball (277) с aiStyle-14 gravity/Expert steering/90% rebound, Golem Fireball (258) с aiStyle-8 collision counter, четырьмя bounce и завершением на пятом impact, а также Skeletron Prime Bomb (102) с нулевым damage в полёте и authoritative 128x128 on-kill explosion. Runtime lifecycle теперь хранит vanilla `Projectile.localAI[0..2]`: на нём source-backed работают фазы Phantasmal Eye (452), Phantasmal Sphere (454), Phantasmal Deathray (455), Fairy Queen Lance (919) и Fairy Queen Sun Dance (923), причём released Sphere получает динамический `extraUpdates = 1`. Phantasmal Deathray generation-safe привязан к Moon Lord Head/Free Eye, как source aiStyle 84 не проходит через обычный `UpdatePosition`, хранит lifetime/scale в `localAI[0]`, а длину луча `localAI[1]` получает из трёх source-style tile `LaserScan`; player collision использует этот же луч и 20-update warm-up. Hallow Boss Rainbow Streak (873), который уже создаётся Empress AI, теперь исполняет source AI_171 drift/homing phases. Для Lance/Sun Dance player collision использует source line hitboxes и точный damage gate `localAI[0] <= 60`, а не маленький storage AABB. Для launcher types 133..144 он также владеет source-backed aiStyle-16 bounce/arming поведением. Когда trusted launcher projectile доходит до vanilla-style `Kill()`, generation-safe termination commit сохраняет trusted owner и final snapshot уже после удаления generation из live-store; bounded same-tick handoff затем применяет к NPC/PvP source-backed `PrepareBombToBlow` AABB 128x128 для I/II или 200x200 для III/IV. В detached projectile snapshot также подставляется source-backed explosion knockback. Поэтому explosion damage остаётся authoritative, хотя live generation уже удалена.

Runtime по-прежнему fail-closed на важных недостающих частях: полном weapon/ammo/provenance coverage за пределами admitted families, включая более поздние rocket-ammo variants; других controlled/channeled families вроде legacy Flying Knife aiStyle-9 path, yoyos/holdouts; localNPCImmunity-aware post-hit reacquisition цели для controlled magic и точной parity transient NPC-флагов `friendly/chaseable/immortal`; полном authoritative учёте mana-cost modifiers/refill validation; multishot и special spawn parameters; полных vanilla projectile AI families; остальных local/static NPC immunity variants; оставшихся exceptional hitboxes/target rules; explosive self-hurt/owner-hit semantics; world/tile-destruction side effects Rocket II/IV; projectile buffs/debuffs; child/on-hit projectile spawn ordering и type-specific on-hit/on-kill effects. NPC-side переход Moon Lord, который переводит уже созданные Phantasmal Sphere в released `ai[0] = -1`, пока остаётся отдельным cross-entity side-effect gap. Поэтому client packet-27 generations вне strict source-backed catalog остаются synchronization/diagnostic state, а не authoritative combat sources.
