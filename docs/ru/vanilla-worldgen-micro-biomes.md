...
Исправление рельс (2026-09-08): индексы `Minecart.Initialize/FrameTrack` теперь задают соединённые прямые участки, уклоны и окончания; прежние нулевые кадры превращали каждую клетку в отдельный рельс. Индекс обратного пути — `-1` (без стрелки). Как в `TrackGenerator.PlacePath`, установка сбрасывает жидкость, окраску и актуаторы клетки рельса, сохраняя тип стены и четыре провода. Начальный просвет — $6\,\text{клеток}$, с перевыбором $5..8\,\text{клеток}$ при `Next(7)==0`; прежняя расчистка трёх клеток могла мешать игроку в вагонетке. До изменения мира проверяется весь максимальный просвет на защищённые объекты. Тесты Small/Medium/Large проверяют взаимные соединения после полного конвейера генерации, а не только количество рельс.

Это исправляет кадры и просвет новых путей, **не** полный паритет маршрутов/RNG `TrackGenerator`. Поиск начала, переписывание истории, выбор и сглаживание тоннелей, преобразование нажимных плит и завершение стен/склонов остаются открыты. Сохранённые миры автоматически не переписываются.
