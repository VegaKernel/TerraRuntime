...
Action row roster содержит ровно **`+ Sandbox`**, затем **`+ Bot`**. `+ Sandbox` открывает typed форму создания sandbox. `+ Bot` принимает настоящий dashboard command binding и по умолчанию создаёт PlayerBot в primary world; double-click по строке бота открывает typed settings window. Настройки бота содержат PlayerBot/NpcBot body, один deduplicated source-verified NPC type, weapon policy, mobility accessories, target и режим Idle/Follow/Guard. Pickup, лечение и поддерживаемые combat buffs теперь являются постоянно включёнными способностями PlayerBot, а не переключателями. Одежда, причёска и допущенный armor set генерируются отдельно для каждого бота вместо выбора UI preset. PlayerBot использует существующие server-player/projectile/world-item authorities; NpcBot является настоящим authoritative NPC presentation/motion actor. В обычной ситуации PlayerBot идёт по земле на уровне защищаемого игрока. Fishron Wings и допущенные mobility accessories остаются traversal-возможностью: контроллер удерживает подъём только когда игрок заметно выше или прямой маршрут перекрыт solid-препятствием, и тогда выбирает действительно более высокую formation point вместо постоянного низкого зависания. Поддерживаемые NPC types ограничены source-backed controlled-motion families: ground fighters, AI_002 flying eyes, AI_005 flyers и обычные pre-wander AI_014 bats. Escort destination NpcBot использует разнесённую per-bot formation и сохраняет replicated vanilla target `255`; тело также имеет server-side zero contact damage и invulnerability, поэтому не попадает в обычные NPC death/loot/progression paths. Поддерживаемые эффекты Archery/Wrath potion для PlayerBot являются trusted server-side combat state и не рассылаются посторонним наблюдателям как packet `55`, потому что vanilla 1.4.5.8 применяет этот PvP-buff packet только к закодированному local player. Неподтверждённая NPC combat semantics остаётся fail-closed и не аппроксимируется.
