...
GodMode — authoritative generation-scoped runtime flag. Он устанавливается только через typed trusted-host/TUI administration boundary, переносится в detached Level-1 transfer state для того же live connection и удаляется при disconnect. На диск он не сохраняется. При включённом флаге authoritative PvP, NPC contact и admitted hostile NPC-projectile damage завершаются до mutation HP/immunity. Поскольку vanilla клиент всё ещё может локально отрисовать `Player.Hurt` до прихода серверного решения, каждый avoided hit сразу повторно утверждает для owning connection текущие authoritative health и movement. Каждый packet-13 movement report в коротком post-hit correction epoch поглощается, чтобы client-local knockback не стал authoritative; peers эти correction frames не получают. Combat text `MISS` остаётся только presentation-эффектом.

Terraria 1.4.5.8 по-прежнему считает lava, drowning и fall damage локально через `Player.Hurt`. Пока эти environmental sources не симулируются TerraRuntime полностью, GodMode player не может уменьшить authoritative HP входящим packet 16: decrease отклоняется, а текущие authoritative health и movement сразу повторно утверждаются для этого connection; тот же короткий packet-13 correction epoch не позволяет environmental knockback вернуться как authoritative movement. Это compatibility guard, а не передача клиенту произвольной health authority.
