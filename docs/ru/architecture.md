...
`BoundedWorkerPool` пробуждает выделенные потоки, не требуя свободного потока в пуле .NET. Синхронно может завершаться только уведомление закрытого канала заданий; само задание исполняется на выделенном потоке, а уведомления о результатах для внешних потребителей остаются асинхронными. Токен отмены сохраняется, пока его источник существует: задача, завершившаяся после ограниченного ожидания при освобождении пула, не обрушает процесс обращением к освобождённому источнику токена. Освобождение пула по-прежнему не может принудительно остановить задание, которое не поддерживает отмену.

Регрессии в отдельных процессах занимают единственный поток пула .NET перед отправкой задания и намеренно завершают задание после предельного ожидания остановки. Обе проверки проваливаются на прежней реализации. Тесты координации кэша секций используют выделенные потоки для блокирующих запросов и наблюдают ошибки этих запросов при очистке; производственный срок ожидания запроса не изменён.
