...
Полный непрерывный бой, валуны Good World, AI/бафф/лечащие сгустки Moon Leech, все сочетания целей мультиплеера, визуальный поворот уничтоженной головы и атаки True Eye остаются открытыми.

## Исправление зависания, Bolt и сфер True Eye

В обычном окне зависания `AI_081` теперь каждый тик заново выбирает ближайшего живого игрока. Зрачок поворачивается к предсказанию его скорости на 20 тиков, открывается до `0.7`, а скорость на одну тридцатую приближается к точке на 200 пикселей выше игрока со скоростью 24. В окне Bolt скорость затухает, меняется масштаб зрачка, а Phantasmal Bolt создаётся из эллипса зрачка в исходные такты 76 и 83. Окно сфер проходит шесть исходных лучей, создаёт Phantasmal Sphere на каждой десятикратной границе, добавляет верхний импульс подготовки на такте 75 и затем выпускает только свои ещё не выпущенные сферы вектором скорости 12. Уничтоженный True Eye сохраняет `ai[0] = -2` на ненулевых записях таблицы атак и возвращается в состояние `0` только в следующем окне зависания. Общий поток случайных чисел также потребляет начальный звуковой бросок AI_081 до проверки связи с ядром. `MoonLordFreeEyeTests` проверяет движение, смену цели, бросок RNG, геометрию Bolt, лучи сфер и обе границы мутаций. Остальные состояния атак True Eye, раздвижение соседних глаз и полные трассы боя остаются открытыми.
