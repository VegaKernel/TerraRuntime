...
## Исправление зависания True Eye

В обычном окне зависания `AI_081` теперь каждый тик заново выбирает ближайшего живого игрока. Зрачок поворачивается к предсказанию его скорости на 20 тиков, открывается до `0.7`, а скорость на одну тридцатую приближается к точке на 200 пикселей выше игрока со скоростью 24. Это заменяет прежнюю искусственную орбиту вокруг ядра. Уничтоженный True Eye сохраняет `ai[0] = -2` на ненулевых записях таблицы атак и возвращается в состояние `0` только в следующем окне зависания. Общий поток случайных чисел также потребляет начальный звуковой бросок AI_081 до проверки связи с ядром. `MoonLordFreeEyeTests` проверяет движение, смену цели, бросок RNG и границу маркера уничтожения. Остальные состояния атак True Eye, выпуск снарядов, раздвижение соседних глаз и полные трассы боя остаются открытыми.
