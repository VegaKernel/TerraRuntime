...
Выбор получателей — это source rule, а не runtime-приближение. TerrariaServer 1.4.5.8 распределяет liquid change set одного tick по network chunks размером $200 	imes 150$ и рассылает каждый chunk только тем клиентам, у которых для этого chunk выставлена запись `TileSections`: `NetLiquidModule.ChunkChanges.BroadcastingCondition` читает `Netplay.Clients[clientIndex].TileSections[ChunkX, ChunkY]`, а legacy-путь `NetMessage.sendWater` применяет тот же тест `TileSections[x / 200, y / 150]`. То есть gate — это section ownership, а не расстояние. TerraRuntime вычисляет тот же предикат по `PlayerSectionStreamingState` — per-connection таблице секций, уже переданных packet `10`.

Тест применяется дважды, по разным причинам. Симулированная ячейка, которую не смогло бы получить ни одно playing connection, вообще не попадает в retention map, поэтому world-wide шум симуляции не может занять ограниченный pending set и вытеснить ячейки, которые клиент реально видит. Тест повторяется при дренировании frame, потому что connection может пересечь границу секции между admission и drain. Frames, отброшенные любой из двух проверок, считаются отдельно как section-filtered liquid updates. Connection без таблицы секций — bootstrap-only endpoints в тестах и terminal harness — сохраняет безусловное поведение.

Authoritative liquid state и порядок его update не меняются. Network detail показывает emitted, pending, coalesced, evicted и section-filtered liquid updates; rolling message table остаётся источником реальной on-wire частоты.
