...
`VanillaItemPlacementDefinition` больше не означает «один из трёх предметов, выписанных руками». Три предмета —
Dirt Block, Stone Block, Sand Block — это всё, что каталог когда-либо нёс, и на любую другую установку клиенту
отвечали коррекцией; именно поэтому подключённый игрок не мог строить. Теперь каталог откатывается на
`VanillaItemPlacementTable1458`, сгенерированную из закреплённого выделенного сервера прогоном
`Item.SetDefaults` по каждому id: **3 231 предмет ставит блок и 292 ставят стену**, причём `createTile`,
`createWall`, `placeStyle` и `consumable` каждого предмета — ровно те, что задаёт исходник.

Рукописные определения по-прежнему выигрывают там, где они есть, потому что несут тайминги использования и
факты инструмента, которых в таблице нет. Тест сшивает оба источника, так что рукописная запись не может
разойтись с собственными значениями исходника, а суммы самой таблицы проверяются — чтобы усечённая
регенерация валила сборку, а не живой мир.

Таблица перегенерируется скриптом `tools/ci/generate_item_placement_table.py` из зонда в
`.cache/itemdefs-probe`; она коммитится как сгенерированный код и проверяется повторной генерацией с диффом.

Для Dirt Block это даёт:

- `TileTypeId = Dirt (0)`;
- `Consumable = true`.

У установки стен раньше не было представления вообще. `TryGetWallPlacement` отвечает за неё, а действие 3
пакета 17 (`PlaceWall`) теперь допускается и применяется по собственным правилам `WorldGen.PlaceWall`:
двухклеточная кромка мира отклоняется, а клетка, где стена уже есть, отклоняется, а не перезаписывается.
