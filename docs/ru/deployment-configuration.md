...
CoreCLR-хост инициализирует общие сборки контрактов до создания первого контекста загрузки host-модуля. Инициализацию нельзя откладывать до callback разрешения сборок: поиск конфигурации Terminal.Gui сканирует загруженные сборки и иначе может образовать цикл ожидания с параллельной загрузкой типов модулей. Идентичности общих сборок, разрешение зависимостей и жизненный цикл выгружаемых модулей не изменены.
