...
`+ Bot` по умолчанию создаёт PlayerBot в primary world через настоящие typed bot operations основного runtime; в production composition кнопка больше не остаётся disabled. Строки bot показывают body kind; double-click открывает typed settings window. Для PlayerBot оставлены automatic/bow/gun/melee weapon policy, mobility accessories, target и режим Idle/Follow/Guard. Pickup, лечение и поддерживаемые buffs включены всегда. Одежда/причёска и допущенная броня генерируются для каждого бота, поэтому UI presets удалены. Для NpcBot показываются по одной записи каждого применимого source-verified NPC type, target и режим движения; player-only loadout/mobility controls скрыты.

PlayerBot переиспользует обычные server-player/projectile/world-item authorities. Его automatic loadout видимо переключается между Copper Broadsword, Wooden Bow и Musket и публикует выбранный slot вместе с use-item control. Дальняя атака использует source-backed скорость боеприпаса и движение выбранного projectile, рассчитывает упреждение по движущейся цели и отклоняется, если tile-aware линия или смоделированная траектория перекрыта тайлами либо жидкостью. Движение публикует vanilla direction controls, автоматически перепрыгивает низкие препятствия и может удерживать source-backed wing flight, чтобы подняться над воздушным препятствием впереди. Разные боты получают разнесённые escort offsets и фазы движения вместо копирования одной координаты. Включённый mobility loadout занимает пять обычных accessory slots предметами Fishron Wings, Soaring Insignia, Terraspark Boots, Magiluminescence и Master Ninja Gear; допущенная horizontal physics применяет проверенный run-speed slice Terraspark и grounded Magiluminescence без выдуманного dash behavior.

NpcBot является настоящим authoritative hostile-NPC presentation/motion actor; доступные types ограничены verified ground-fighter, AI_002 flying-eye, AI_005 flyer и обычными pre-wander AI_014 bat motion families. Follow/Guard использует разнесённую escort point вместо центра игрока, координаты остаются приватным actor intent, а реплицируемый vanilla NPC target остаётся `255`. Такое разделение обязательно: немодифицированный client получает contact damage из NPC type и не может получить per-instance zero-damage override сервера. Authoritative тело сохраняет zero contact damage и invulnerability до появления bot-specific death/drop semantics; неподтверждённая NPC combat semantics остаётся fail-closed.
