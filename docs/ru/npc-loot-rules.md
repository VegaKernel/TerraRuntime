...
Настоящий конвейер смерти Скелетрона теперь передаёт маркер RedHat подтверждённого состояния головы существующему расчёту добычи. При `ai[3] == 1` пять зарегистрированных в оригинале декоративных предметов (`5624`, `5625`, `5626`, `5737`, `5628`) выпадают в мир в Classic, Expert и Master; локальный маркер, используемый рукой, не включает награды головы. Девять регрессионных сценариев конвейера проверяют доставку packet 21 всем зарегистрированным игрокам и отсутствие повторных наград при устаревшем повторном убийстве. Это закрывает ранее неподключённое условие, но не означает полного паритета боя, координат и всех модификаторов добычи.
