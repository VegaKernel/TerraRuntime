...
Гейт сканирует `src/TerraRuntime.Gameplay`, а также gameplay-owned C# в `src/TerraRuntime.Core`, `src/TerraRuntime.Application` и `src/TerraRuntime.World`. Protocol, persistence, snapshot и world-generation adapters, чьи имена явно обозначают boundary ownership, исключены: на этих границах raw representation допустим.
