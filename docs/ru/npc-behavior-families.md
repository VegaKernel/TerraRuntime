...
- Angry Bones и Armored Skeleton дополнительно включают закреплённый grounded close-range lunge (`|dx| < 100`, `|dy| < 50`, движение к цели, удвоение X с clamp до `±3`, Y = `-4`). World-motion также передаёт реальный NPC type в AI_003 door-pressure policy, поэтому type-specific pressure/reset правила больше не вычисляются как будто любой fighter является Zombie. Goblin Peon type `26` теперь владеет отдельной authoritative-веткой разрушения: целиком удаляются закрытая дверь или высокий шлюз, запертые двери Подземелья остаются защищёнными, выпадает предмет исходного стиля, а подтверждённое разрушение реплицируется packet 17. Обычное открытие остаётся packet-19 mutation, а занятость высокого шлюза в production composition вычисляется по живым игрокам и NPC.
- `VanillaBatNpcBehaviorStrategy` владеет обычным срезом AI_014 для Cave, Jungle, Hell, Giant, Illuminant, Ice и Lava Bat, Giant Flying Fox и Spore Bat. Их закреплённые definitions и source-ordered движение покрывают отскок при столкновении, обновление ближайшей цели, отдельные коэффициенты Hellbat, выход из жидкости и pursuit/wander clocks; runtime targeting передаёт актуальные wet-state и line of sight. Для пары Vampire добавлены source distance, lower-edge и line-of-sight переход от летающего type 158 к humanoid type 159, а также обратный переход на расстоянии 300 pixels. Оба превращения сохраняют нижнюю границу, пропорционально переносят life, сбрасывают AI-local defaults и выбирают цель заново. Стреляющие варианты AI_014 остаются fail-closed.
- Та же явная стратегия AI_014 теперь допускает Slimer и Purple Slime minion Королевы слизней. Slimer получает один общий проход ускорения, а minion — отдельный steering-профиль `0.35/0.30`, `6/5`; ни одному из них не приписывается второй проход и wet-ветка летучих мышей.
- `VanillaFishNpcBehaviorStrategy` владеет всеми тринадцатью типами AI_016: Goldfish, Corrupt Goldfish, Piranha, Shark, Angler Fish, Arapaima, Blood Feeder, Crimson Goldfish, Gold Goldfish, Pupfish, Dolphin, Pufferfish и Orca. Runtime передаёт актуальные wet/LOS-факты игрока, нижний slope, соседние liquid/floor, свободный вертикальный путь и точный результат `GetWaterLine`. Source-ordered автомат покрывает passive/hostile targeting, разворот при столкновении, три pursuit-профиля, idle depth oscillation, сухие server-RNG прыжки, breach/surface состояния Dolphin и inflation/localAI decay Pufferfish. Lighting, rotation и звук Dolphin остаются presentation-only.
