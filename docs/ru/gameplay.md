...
Обычный AI69 Duke учитывает геометрию mounted-player из TerrariaServer: обычный height boost маунта и отдельные размеры Rat, Bat и Pixie участвуют в центрах цели, ocean gates и overlap checks. Spawn defaults Duke также сохраняют source-арифметику life/damage Classic/Expert/Master, multiplayer balance, Good World scale/hitbox 1.3 и половинный hitbox Celebration Mk10; Good World имеет приоритет, если выставлены оба seed flags. AI69 использует физическое тело Duke для target range, centers и phase-three relocation. Остаются открытыми live wind/weather, другие phase details и официальный client encounter acceptance.
