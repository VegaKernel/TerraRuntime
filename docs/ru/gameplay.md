...
Обычный AI69 Duke Fishron теперь получает подтверждённые ширину мира и surface через существующую NPC authority. Ocean gates используют левый верхний угол игрока и строгие сравнения800/6400/surface/правого края. Enrage сокращает hover до10 ticks, добавляет6 к скорости рывка, заменяет bubble-specials, меняет damage/defense и передаёт признак Cthulhunado bolt; возвращение к океану восстанавливает обычную фазу. Обычный Classic/Expert/Master defDamage равен100/140/210 до phase/enrage. Без границ мира root step/projectile plan отклоняется. Эти source-backed части не закрывают distant-target/despawn ordering; AI69 теперь обновляет удалённую удерживаемую цель перед проверкой retreat на 5600 пикселях, уменьшает вертикальную скорость, ограничивает lifetime и сбрасывает phase clock, затем продолжает initialization/rotation/phase dispatch в том же update. Duke учитывает геометрию mounted-player из TerrariaServer: обычный height boost маунта и отдельные размеры Rat, Bat и Pixie участвуют в центрах цели, ocean gates и overlap checks. Spawn defaults Duke также сохраняют source-арифметику life/damage Classic/Expert/Master, multiplayer balance и половинный hitbox Celebration Mk10. Остаются открытыми остальные special seeds, live wind/weather, другие phase details и официальный client encounter acceptance.
