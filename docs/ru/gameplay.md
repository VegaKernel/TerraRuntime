...
Обычный AI69 Duke учитывает геометрию mounted-player из TerrariaServer: обычный height boost маунта и отдельные размеры Rat, Bat и Pixie участвуют в центрах цели, ocean gates и overlap checks. Spawn defaults Duke также сохраняют source-арифметику life/damage Classic/Expert/Master, multiplayer balance, Good World scale/hitbox 1.3 и половинный hitbox Celebration Mk10; Good World имеет приоритет, если выставлены оба seed flags. AI69 использует физическое тело Duke для target range, centers и phase-three relocation. Runtime хранит source current/target wind раздельно, применяет сглаживание с множителем дождя и передаёт live target в packet 7. Его серверный weather-срез также запускается один раз на каждую единицу day rate, замирает вместе со временем, учитывает Freeze Wind после сглаживания, использует source-переходы `windCounter`/`extremeWindCounter` и ограничивает модуль target значением `.35` до появления игрока с 120 HP. Тот же владелец теперь сохраняет `raining`, `rainTime` и `maxRaining`: eligible игрок с 120 HP может запустить обычный source-shaped интервал, его draws длительности/силы и повторные выборы во время дождя следуют проверенным веткам, Freeze Rain его останавливает, packet 7 получает live strength, checkpoint восстанавливает состояние, а town shelter scheduling читает live-флаг. Coin Rain, ladybug boost, Lantern Night, forever-rain и полная интеграция общего RNG остаются открытыми. Другие phase details и official client encounter acceptance также остаются открытыми.

Empress of Light: planner `AI_120` в state 4 создаёт кольцо Fairy Queen Lance. Classic: 4 якоря на $300\,\text{пикселей}$ с горизонтальной поправкой `0.5f`; Expert/дневная ярость: 5 якорей на $450\,\text{пикселей}$. План проверяет лимит $2400\,\text{пикселей}$, нормализованную скорость цели с fallback `UnitY` для неподвижной цели, прогноз на $90\,\text{тиков}$ и смещение spawn на $30\,\text{тиков}$. States 2, 4, 11 и 12 также повторяют source extra-player volley: кандидат должен иметь interaction credit с этим NPC, попасть в текущую modulo-three rotation, быть активным и не мёртвым и находиться не дальше $2400\,\text{пикселей}$; primary target всегда пропускается. Presentation, все ветки сложности и полный parity encounter остаются открыты.
...
State 6 Sun Dance выпускает 6 лучей в Classic и 8 в Expert или при дневной ярости; число не зависит от второй фазы.

State 7 сохраняет шесть исходных геометрий lance-wall, дальность цели $3200\,\text{пикселей}$, cadence Expert/ярости и source predictive aim blend $75\%$. Classic float-loop $1/13$ выпускает 14 lance; Expert-loop $1/18$ перескакивает включительную границу и выпускает 18.

State 11 direct lance отклоняет первичную цель дальше $2400\,\text{пикселей}$. State 12 вращает исходный вектор $(0,-20)$ в своём $50$-тиковом окне; Expert/ярость использует cadence $4$ тика, Classic — $6$, независимо от фазы.

Состояния Empress 4, 5, 6 и 7 сохраняют source total durations $120$, $72$, $300$ и Classic/Expert $260/280$ тиков до phase/Expert reductions.

Перемещение Empress теперь повторяет `NPC.SimpleFlyMovement`: при смене направления каждая компонента получает второй исходный импульс, а внутри радиуса $40\,\text{пикселей}$ босс не корректирует скорость. States 2 и 3 используют смещения $(-150,-250)$ и $(150,-250)$; states 4, 5, 7 и 11 — $(0,-350)$. State 6 использует $(-80,-500)$ со скоростью $3.6$ и ускорением $0.35$, state 7 — $4.8$ и $0.5$. Dash states 8/9 сохраняют исходный полёт 40 тиков, exit-lerp к горизонтальной скорости $+/-50$ и затухание на тиках 40/90. State 12 стартует вверх с $(0,-12)$ и каждый тик затухает на $0.95$.
...
Для 35 предметов допущены только world-drop dimensions и natural-prefix families. Независимое сравнение с официальным сервером совпало для 3500 item/seed prefix outcomes и следующих RNG draws; сохранены исключения округления Venus Magnum и Heat Ray. Открытие храмовой двери Temple Key через packet 52 action 2 теперь входит в authoritative tile/inventory path: он расходует первый ключ 1141 в слотах 0..57, открывает проверенный трёхстрочный контур двери, отправляет packet 52 остальным игрокам и source-квадрат тайлов 2×2. Действия packet 19 0 и 1 также входят в tile authority: открытие выполняет source-подобное преобразование 1×3 в 2×3, а forced-close проверяет открытый контур 2×3 и сохраняет три упорядоченных source-выбора случайной frame-column; оба ретранслируются только остальным игрокам. Его forced tall-gate действия 4 и 5 также проверяют полный объект 1×5 и изменяют только его тип перед peer-only relay. Packet 19 action 2 теперь разрешает замкнутый контур type 387, резервирует оба поддерживаемых destination drop до мутации, удаляет допустимые клетки `tileCut`/drip через authority, затем выполняет source type shift и peer-only relay. Назначения, для которых полный `KillTile` drop path ещё не смоделирован, отклоняются до изменения тайлов. Void Bag fallback, оставшиеся trapdoor `KillTile` families, общие luck/coin/heart rules и содержимое мешков остаются долгами. Эти награды и существующие AI slices не доказывают полную проходимость vanilla.
...
Ещё три механики `Liquid.Update` теперь authoritative, и именно они вместе дают миру затихнуть, а не просто устояться в среднем.

Трёхклеточная ветка averaging несёт единственный random draw во всём методе: `if (num == 254f && WorldGen.genRand.Next(30) == 0) num = 255f`. Это единственный выход из замкнутого pocket, вода в котором усредняется ровно в 254, а клетка сверху продолжает передавать вниз по одной unit через намеренное full-source исключение — поэтому runtime без этого draw оставляет такой pocket переключаться между 254 и 255 на весь срок жизни мира. Draw проходит через `IVanillaLiquidRandom1458`, так что тесты закрепляют оба исхода.

У каждой ширины averaging свои условия записи и wake-up, и они не совпадают. Ветки на семь и пять клеток пишут отличающегося соседа и будят его, а затем будят всех соседей ещё раз, если собственный pre-update amount источника отличается от нового level. Ветки на четыре клетки сворачивают оба слагаемых в одно условие, которое заодно гейтит запись. Трёхклеточная ветка работает строго по собственной разнице соседа. Двухклеточные ветки пишут по разнице соседа, но будят только по разнице источника — именно это позволяет устоявшейся паре остаться устоявшейся: retired `255`, придавленный обратно к среднему `254`, исправляется без оживления. Горизонтальный averaging вообще никогда не будит строки сверху или снизу; единственные вертикальные wake-ups принадлежат шагу течения вниз и собственному `AddWater(x, y - 1)` в kill tail.

`Tile.skipLiquid` тоже воспроизведён. Перенос вниз помечает обе свои клетки, и обычный цикл `Liquid.UpdateLiquid` затем один раз пропускает помеченную клетку — снимая метку и не трогая её delay и kill state — вместо её update. Quick-fall цикл всегда делает update и снимает метку после него, а успешный `Liquid.AddWater` снимает её, занимая свежий slot. Флаг — это transient simulation state, который никогда не попадает в сохранённый мир, поэтому он живёт вместе с delay/kill work state, а не на tile.

Material dispatch в начале метода вложенный, а не плоский, и теперь вложенность точная: клетка запускает только свой собственный `LiquidCheck`, а для каждого материала, которым она не является — в source-порядке lava, honey, shimmer — вместо этого будит соседние клетки, которые им являются. Поэтому lava не будит ничего, honey будит соседнюю lava, shimmer будит соседние lava и honey, а water будит все три.

В измерении против pinned official assembly на регионе, снятом из сгенерированного large world, два движка теперь согласуются по convergence: official `Liquid.UpdateLiquid` устаивает его за $359$ ticks, runtime — за $386$. Сгенерированный мир 8400x2400 доходит до нуля active и нуля buffered liquid cells и там остаётся, тогда как раньше в нём держались десятки тысяч вечно осциллирующих клеток. Два остаточных расхождения известны и не закрыты: runtime набирает около $0.8\%$ liquid mass за это окно устаивания, тогда как official engine теряет около $0.04\%$ — оба через собственный неконсервативный full-source перенос vanilla, — и forced settling через `Liquid.stuckCount`/`StartPanic` по-прежнему отсутствует.
