...
TerraRuntime публикует время работы persistence через неизменяемые snapshots operations API, а встроенный World screen TUI показывает три последних времени в миллисекундах. Измерения выполняются на границах владения, а не через чтение изменяемых внутренних объектов сохранения из TUI или другого фонового наблюдателя.
