...
| `Terminal.Gui` | 2.4.17 | **Yellow-green / runtime UI** | Pinned for the opt-in local TUI. The 2.4.16 line fixed a trimming/NativeAOT configuration regression, so upgrades remain gated. TerraRuntime exercises a real `--tui-smoke` with the ANSI driver in normal CI and both native RID jobs. CoreCLR and Linux NativeAOT have passed this exact path; keep the dependency under review until an uninterrupted Windows NativeAOT publish + TUI smoke also passes, then retain the same gate on every upgrade. |
