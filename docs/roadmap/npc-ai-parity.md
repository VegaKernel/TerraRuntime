...
- [ ] add Hardmode, event and endgame bosses;
  - Hardmode boss bring-up now has explicit authoritative state families for Queen Slime, the mechanical bosses, Plantera, Golem, Duke Fishron, Lunatic Cultist, Empress of Light and Moon Lord; linked Plantera/Golem/Moon Lord parts, Duke Detonating Bubble, Cultist clones and the implemented boss projectile intents are dispatched through the runtime rather than metadata-only fallback.
  - This gate remains open: Duke AI 71 Sharkron/Sharkron2, Cultist Ancient Vision/Dragon children, specialized Cultist/Empress/Moon Lord projectile behavior, exact ritual/enrage/death edge transitions, and differential fixtures against the pinned 1.4.5.8 server still need authoritative coverage before full parity can be claimed.
