...
- [x] Remove `VanillaTileInteractionItemFacts` after migrating remaining callers to `VanillaDefinitionCatalog` directly.
...
- [ ] Decompose `ServerRuntimeState` by real world-owned responsibilities while preserving one authoritative writer. The local 2026-09-04 slice removes the `ServerRuntimeState -> ServerRuntimeComposition -> ServerRuntimeState` construction cycle: world-scoped tick/command counters and generation-safe player snapshot aggregation are explicit collaborators, NPC/projectile/tile composition has no central-state back-reference, and object placement no longer re-enters `ServerRuntimeState.Apply` for its inventory decrement. Keep open until full solution tests/CI validate the completed boundary on `main`.
...
- [x] Extract authoritative client-player command application, inventory and transfer-profile lifecycle plus player metrics from `ServerRuntimeState` into `PlayerAuthority`; keep it on the existing world writer. The 2026-09-04 cleanup additionally moves generation-scoped PvP/PvE immunity deadlines into `RuntimePlayerDamageImmunityStore` and splits combat/transfer implementation files without introducing another player-state owner.
...
- [x] Extract non-town NPC command application, AI/actor/archetype lifecycle, network combat/catch flow and town-NPC coordination from `ServerRuntimeState` into the world-owned `NpcAuthority`. The 2026-09-04 follow-up keeps one combat-pipeline owner but splits ingress/commit, loot materialization and boss/shared-life/death progression into responsibility-focused partial files.
...
- [x] Keep projectile-to-player combat under one authoritative pass while separating orchestration, explosion processing and hostile collision geometry into responsibility-focused partial files; this creates a clean extension point for future trail/old-position collision without another state owner.
