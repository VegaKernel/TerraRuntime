...
2026-09-21 Snow Moon waves 6-10: `NPC.Spawner.SpawnAnNPC` now selects source Everscream, Santa-NK1, Ice Queen and ground-fighter branches through wave ten, retaining every active-type cap and RNG draw after the wave-wide 1-in-30 special check. Unsupported special selections fail closed instead of falling back to an ordinary hostile. Waves 11-20, their no-spawn branches, and remaining special-AI NPCs remain open.
...
2026-09-20 Mechdusa Probe attachment: AI_005 now applies the live Destroyer hitbox/rotation and source `26 × ai[3]` orbit with invulnerability, while retaining the live Mech Queen Prime velocity. Its timer is three updates per tick, reaches 360, and fires the speed-8, target-velocity-leading pink laser from the pre-attachment snapped source center. Out-of-range Destroyer slots reacquire the first active head; vanished Prime or in-range-reused anchors clear the attachment marker and invulnerability. Focused regressions cover the orbit, state, boundary, projectile and recovery branches. Other combined AI and encounter parity remain open.

2026-09-20 Mechdusa Twins orbit: Retinazer and Spazmatism now replace only their phase-one hover steering while Prime is live. They orbit Prime's physical center minus `14` with source offsets `(-112.5,-187.5)` and `(112.5,-187.5)`, velocity-X rotation `× 0.025`, speed cap `14`, source `59/60` and `4/5` smoothing, and the `1200` phase timer boundary. The focused runtime regression covers both physical centers, rotation, smoothing and the Spazmatism Cursed Flame `ai[3]` boundary. The live Prime anchor now also enables projectile reflection during Twin transformation states `ai[0]=1/2` and the inverse-smoothing phase-two hover (`4/5` Retinazer, `59/60` Spazmatism) through its `1200` boundary. Projectile ordering and later shared encounter state remain open.

2026-09-20 Mechdusa Destroyer head orbit: the AI_037 post-motion override now derives Prime's center minus `14`, rotates its `(0,100)` anchor by `Prime.velocity.X × 0.025`, transfers Prime velocity to the final position, clears Destroyer velocity and sets the source rotation. The targeted runtime regression verifies physical dimensions, target refresh and all final kinematic fields. Segment attachment, anchor-loss behavior and complete encounter parity remain open.

2026-09-20 Mechdusa Destroyer segment geometry: AI_037 now traces up to nine predecessor links back to the head, applies the source integer-scaled vertical offset and compresses the link gap by index. Target refresh, first-link position, rotation and velocity are regression-covered. Missing-parent recovery and complete encounter parity remain open.
...
2026-09-15 Prime ranged arms: source AI35/36 movement, target handoff, rotation, shot clocks and orphan removal now execute with committed RNG/projectile effects. Original Windows/Linux matrices cover 3,456 baseline + 6,912 geometry/target calls, 96 eight-call sequences and 60 parent cases per platform. Laser100 physical size is corrected to 7x7 after source scaling. Acceptance confirmed on 2026-09-16: focused 10,590 tests, seven negative controls, full 90,031 tests with complete XML and exit 0, and all 14 exact-commit workflows for `2419b562` passed, including Windows/Linux NativeAOT with all five smoke paths. Independent Saw/Vice reference captures are prepared; their runtime implementation remains open. Melee arms, full encounters and remaining gameplay/network parity stay open. See [NPC ownership](en/npc-runtime-ownership.md).

2026-09-15 NPC creation arithmetic: 28,800 independent Windows/Linux creation cases now verify platform-specific integer conversion, life/damage/knockback rounding and multiplayer balance for 15 types. Old creation/RNG/phase matrices gained independent Windows captures; Prime daytime speed also matches the original Windows expression boundaries. Focused 63,125 tests pass. Full 79,489 tests, Windows NativeAOT publish and five smoke paths pass; exact-commit Linux acceptance awaits CI. Other profiles, full encounters and remaining gameplay/packet parity remain open. See [NPC ownership](en/npc-runtime-ownership.md).

2026-09-15 RedHat Skeletron head: source hover/spin/rotation, hand-count reflection, bounded caster summons and localized packet-82 taunts now execute in accepted RNG order. Target reacquisition and skull-before-hover-retarget ordering match new originals. Source-scoped NPC creation rechecks ownership after context/Good World RNG callbacks; heads and caster spheres suppress stale child publication. Retained original head/terrain/transition/coupled/target/flag/edge/packet matrices drive3787 head-effect tests plus2 caster guards. Focused 25,905 and full 50,689 tests pass; eight negative controls detect regressions. Windows NativeAOT publish and all five smoke paths pass; Linux acceptance and all 16 exact-commit workflows passed for efe902f7. Full encounters, outer NPC lifecycle, fractional Windows creation and remaining gameplay/network parity stay open. See [NPC ownership](en/npc-runtime-ownership.md).

2026-09-15 Skeletron skull: four RNG draws and allocation moved out of speculative planning into the accepted source effect. Physical spawn anchor, refreshed target, double health threshold and Windows/Linux arithmetic are source-backed. Separate384-call captures per platform drive1165 checks of exact velocities, host spawn fields/RNG and stale-source boundaries. Full46900/46900passed; WindowsNativepublish/five smokes andfive negative controls accepted. Full head RedHat effects, fractional Windows creation and encounter/network parity remain open. See [NPC ownership](en/npc-runtime-ownership.md).

2026-09-15 Dark Caster AI: type32 target/timers/RedHat inheritance, bounded random teleport and ordered sphere creation now have independent original evidence. Final AI fields publish only after accepted random selection; newer revisions/generations cannot be overwritten. Windows XNA/Linux FNA player-rectangle behavior is selected by host platform and verified against both originals.7599 caster tests including48 edge queries; full45735/45735pass and seven negative controls detect regressions. Head RedHat effects, other casters and full encounter/gameplay/packet parity remain open. See [NPC ownership](en/npc-runtime-ownership.md).
...
2026-09-15 RedHat combat/loot: shared source marker now drives hand AI, authoritative post-defense/critical damage mitigation and the committed Skeletron death reward condition.1760 original strikes and54 original predicate calls retained; nine death-pipeline cases verify five vanity drops across modes and stale-kill rejection. Damage/loot negatives fail186/3. Head RedHat AI/minions, caster/sphere definitions, other damage/loot modifiers and full encounter/network parity remain open.

2026-09-15 Skeletron hand variants: RedHat damage/timers/wind-up/dashes and parent-marker retention now match source; active AI-style-11 parents include Dungeon Guardian base geometry. Orphans deactivate in the same committed pass without an intermediate active update or combat loot. Retained1296 RedHat calls and288 parent traces (1248calls),3810 focused tests; parent/deactivation/RedHat negative controls fail48/193/1300. Guardian AI/difficulty scaling, head RedHat, target-loss details, rotations and full encounter/network/RNG parity remain open. This supersedes hand-variant gaps in the historical entries below.

2026-09-15 ordinary Skeletron hands: physical alignment/dash centers, sprite/dash facing, parent localAI copy, state-scoped flee lifetime and player-top-left distance exits match2220 original calls. Vanilla creation now initializesDirectionY1 while exactstorage/updates preserve explicitzero. Oldhandimplementation1482fails; direction/distance/localAI/player-corner controls1933/24/12/48fail. ParentAI is frozen; orphan removal, other parentAIstyles, RedHat and full encounter/network/RNG parity remain open.
...
2026-09-15 Skeletron Prime continuation: spawn context now covers the head and four arms; retained base damage/defense survive live AI overrides and reset on type/generation changes. Head phases use physical centers, original distance guards and float operation order. Independent395 creation/408 AI cases and runtime/native/negative checks cover the slice. Full limb AI, Mechdusa, rotations, complete encounter/network behavior and other NPC/item/packet parity remain open. See [NPC ownership](en/npc-runtime-ownership.md).

2026-09-15 NPC spawn context: each vanilla allocation now captures effective difficulty, live owned player count and good-world seed facts through the world authority. Destroyer head/body/tail and Probe use independently verified physical/visual scaling, health, damage and player-count rules; 240 applicable mode cases and76 fractional-difficulty cases match original creation output. The original32 chain cases now include good-world dimensions/health, and actual runtime tests cover same-pass children and later player-count changes. Effective AI Expert/Master flags now include the good-world level increment. Other NPC context scaling, Journey controls, ghost projection, broad combat/loot and complete parity remain open; see [NPC ownership](en/npc-runtime-ownership.md).

2026-09-15 Destroyer chain creation: the committed head call now creates the complete body/tail batch with head-based Start, predecessor/root links and the original full-table sentinel. Thirty-two independent original head-call cases cover ordinary/good-world chain topology, four root slots and occupancy/replacement cases; ordinary cases also verify spawn state. A world-pass regression proves the complete chain exists before the first body AI and newly allocated slots run immediately. Restoring the old planner fails all33 tests. Good-world spawn defaults/RNG, Mechdusa, full motion/combat, packet23 cadence and other fixed worm batches remain open. See [NPC ownership](en/npc-runtime-ownership.md).

2026-09-15 NPC Start-index extension: the shared vanilla allocator and committed spawn intents now carry a bounded NewNPC.Start index, with ascending inclusion and reverse exclusion verified against25056 additional original decisions. Skeletron hands, Skeletron Prime arms, the Wall of Flesh initial eye/Hungry batch and later Hungry, Plantera's three initial hooks, Golem's detached head, the Moon Lord shell, and Lunatic Cultist ritual clones use their parent slot as in the source; regressions confirm those batches allocate above a root in slot 10 despite earlier free slots. Other producers, especially batched worm chains and remaining boss children, still need individual Start mapping and combined simulation evidence. This supersedes the allocator-API Start limitation in the earlier entry below; it does not close full NPC parity.

2026-09-15 NPC allocation follow-up: vanilla creation now observes the200-slot table, source type-specific direction/slot-zero exclusions, two-update protection owned by world ticks, and replacement fallback with generation advancement. Queen Bee minions carry the source replacement flag. An original6264-decision fixture plus actual NewNPC reuse traces and negative controls cover these rules. Explicit storage slot construction remains separate. Arbitrary Start offsets, pending-spawn network flush cadence and other replacement-flag producers remain open. This supersedes the spawn-protection limitation in the earlier Moon Leech note below; full NPC/boss/gameplay parity remains unfinished.
...
Full-playthrough continuation (2026-09-20): Plantera and Golem route NPC-specific Classic/Expert/Master death rewards through the existing authoritative combat and world-item/instanced-loot owners. Packet 52 action 2 has an inbound owner: it consumes the first ordinary-inventory Temple Key1141, validates and unlocks the source three-row door footprint, echoes packet52 to other peers and sends the source 2x2 tile square. Packet19 actions0/1 normal-door opening and forced closing are also connection-owned: opening uses the source-shaped 1x3-to-2x3 transform, closing validates the 2x3 open footprint and makes the source three ordered frame-column draws, with source peer-only relay. Its forced tall-gate actions4/5 validate a full1x5 object, preserve its frame data, switch only the type and relay peer-only. Packet19 action2 now resolves the type-387 source, reserves both supported destination drops before either mutation, removes admissible tileCut/drip cells, then shifts and relays the trapdoor. Unsupported full KillTile destination families reject before any mutation. Focused wire, ingress, inventory-order, tile and replication regressions pass. This does **not** close end-to-end progression: Void Bag key fallback, remaining packet19 trapdoor KillTile families, bag opening, complete item/weapon/projectile semantics, global loot/luck, remaining boss rewards and full dungeon/worldgen parity remain open. See [death-loot scope](en/gameplay.md#plantera-and-golem-death-rewards).
