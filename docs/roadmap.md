...
Current TZ-32 slice owns a conservative pickup transition: every five ticks the server assigns the nearest eligible live player with an empty ordinary slot, publishes packet 22, and accepts packet-21 removal only from that exact reservation owner. Unsupported stacking, special magnets and alternate storage remain fail-closed, so the broad ownership/pickup checkboxes stay open. Cross-runtime transfer now treats vanilla slot 58 as `Main.mouseItem`: it moves an occupied cursor stack once into an empty slot 0..49 or aborts before detach, explicitly clears destination slot 58, and rejects landing-gate packet-5 echoes that could restore the stale cursor image.
...
- [ ] Complete NPC/projectile side effects. Trusted admitted hits now commit server-resolved NPC/PvP damage, consume source-backed penetration, apply ordinary shared owner/NPC immunity for admitted multi-hit families, apply permanent local NPC immunity for grenade-launcher variants 133/136/139/142, apply Flamelash/Rainbow Rod's source-backed 12-tick projectile-local NPC immunity, and apply the source-backed 40-tick projectile/player immunity baseline in PvP. Launcher and Moon Lord Eye/Sphere Kill() damage is replayed from generation-safe same-tick termination handoffs after the live projectile is removed. Sharknado Bolt (385) Kill() stages source-backed Sharknado (384) or terrain-anchored Cthulunado (386) children with classic/expert damage and inherits the exact originating `NpcHandle` generation; world-bounds removal does not manufacture children. Live aiStyle-64 tornado parents now stage the next segment only after their exact projectile generation commits and revalidate the exact source `NpcHandle` generation before publishing either the segment or its source-interval Sharkron/Sharkron2 NPC. Cultist Ice Mist (464) uses the same post-commit boundary for source 30/60/90/120 child emission; Cultist Lightning Orb (465) revalidates its exact NPC provenance before source-ordered, line-of-sight-gated Arc (466) emission at 30/60/90/120/150; and Cultist Fireball/Clone (467/468) replay 176x176 Kill() damage. Buff/debuff mutation, including the Fireball 467 On Fire! application, remaining local/static immunity variants, explosive self-hurt, unsupported explosive object-destruction side effects, exact same-pass physical-slot child update ordering and other type-specific effects remain open.
...
- [x] Operator bot policy is isolated in `TerraRuntime.Application.Bots` with source-pinned content facts in `TerraRuntime.Gameplay.Bots`; PlayerBot owns Idle/Follow/Guard, target/PvP mirroring, stuck recovery, source-backed wing flight, low-obstacle ascent, bounded ammo/healing/useful-potion pickup and source-backed consumable use.
- [x] PlayerBot automatic combat visibly switches between Copper Broadsword, Wooden Bow and Musket through the authoritative selected-slot/use-item path. Ranged admission uses exact ammo launch magnitude, admitted projectile substep/gravity motion, moving-target prediction and tile/liquid obstruction checks; close direct melee reuses the player-owned NPC combat finalizer.
- [x] NpcBot uses a real authoritative hostile NPC presentation/motion actor for verified controlled-motion presets, with Idle/Follow/Guard positioning and stuck recovery; current families include ground fighters, AI_002 flying eyes, AI_005 flyers and ordinary pre-wander AI_014 bats. Follow/Guard target coordinates remain internal while replicated vanilla target is `255`. Bot NPC bodies are zero-contact and invulnerable until bot-specific death/drop semantics exist, and unsupported NPC-owned offensive Guard remains fail-closed.
...
- [x] Shipping TUI composition supplies real bot operations to `+ Bot`; PlayerBot-only loadout/flight/inventory settings are hidden for NpcBot.
