...
- [x] NpcBot uses a real authoritative hostile NPC presentation/motion actor for verified controlled-motion types, with Idle/Follow/Guard positioning and stuck recovery; current families include ground fighters, AI_002 flying eyes, AI_005 flyers and ordinary pre-wander AI_014 bats. The UI deduplicates types. Follow/Guard uses a separated per-bot escort point while replicated vanilla target is `255`; the authoritative body remains zero-contact and invulnerable. Unsupported NPC-owned offensive Guard remains fail-closed.
...
- [x] Shipping TUI composition supplies real bot operations to `+ Bot`; its actual `Command.Accept` binding is acceptance-tested through the operations boundary. Only body-applicable fields are visible. Clothing/armor presets and pickup/consumable toggles are removed: visual identity is generated per bot and the quality-of-life capabilities are always enabled.
