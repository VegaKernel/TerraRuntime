...
- [ ] Complete vanilla projectile gameplay-AI families. `BasicArrow`, `Thrown`, source-backed Enchanted Boomerang type 6, the admitted Skeletron/Deerclops families, Star Cannon Star type 955 (`aiStyle 5`), Super Star type 728 (`AI_151`) and Super Star Slash type 729 (`AI_152` gameplay branch) are explicit simulation profiles; known unsupported families still fail closed. Natural Falling Star type 12 is definition-cataloged but intentionally not profile-admitted yet because its non-remix daytime `damage == 1000` kill gate depends on authoritative world-time/remix state that is not wired into this projectile stepper. Super Star type 728 owns its source-backed `SummonSuperStarSlash` child creation and local NPC immunity, and the For-the-Worthy star reflection is interleaved before damage. Type 729 is now combat-admitted: its `extraUpdates=2` execution commits each of the three local states authoritatively and runs reflection/NPC/PvP interaction between subupdates, matching the pinned `Projectile.Update -> Damage()` ordering without publishing an ordinary packet-27 update for every positive intermediate state.
...
- [ ] Complete entity collision. Tile/world collision exists, and deterministic authoritative passes select NPC and hostile legal PvP player targets for trusted admitted friendly projectiles with generation-safe ownership/team gating. Collision now runs after every committed local `extraUpdates` subupdate, so Super Star Slash type 729 can hit along its three-step per-tick path rather than only at the final position. The admitted PvP slice reproduces general 8-tick PvP immunity, generation-safe per-projectile/per-player 40-tick `Projectile.playerImmune[]`, dodge resolution and the current type-specific `Colliding` gate for Bone Shard. Remaining work is unsupported projectile-family collision hooks/hit shapes, owner-hit checks and other exceptional target rules.
...
- [ ] Complete spawn ordering. The projectile phase walks the live physical table in vanilla slot order `0..999` rather than a pre-pass projectile snapshot: a child allocated into a later unvisited slot is simulated again in the same global tick, while allocation into an already-visited slot waits until the next tick. Within one physical slot, each local `extraUpdates` state is committed before reflection/NPC/PvP interaction, and surviving interaction mutations feed the next subupdate. NPC candidates are generation-safe live-rechecked so earlier-subupdate/earlier-slot kills or replacements are not reused. The admitted Confetti Flask child type 289 retains its pinned combat-mutation order, and Super Star type 728 spawns type 729 after committed NPC hit/immunity/penetration side effects through the normal first-free-slot allocator. Remaining ordering work is kill/on-kill child families, full slot-pressure/oldest-projectile replacement parity and the other child families.
- [ ] Complete NPC/projectile side effects. Trusted admitted hits commit NPC damage/death through the existing pipeline and consume source-backed penetration. In PvP, a collided hit that dodges still consumes penetration and starts the projectile-local 40-tick target cooldown, while a globally immune player is skipped before those side effects, matching `Damage_PVP`. Current admitted type-specific status, server-confirmed `meleeEnchant`, Frost-armor `frostBurn` and `magmaStone` on-hit state are server-owned, and Hallowed `Player.OnHit` protection acquisition is modeled for admitted hits. Confetti child creation is server-owned; Super Star type 728 reproduces generation-local `localNPCHitCooldown=-1`, source-ordered post-hit type-729 slash spawning, and type 729 now consumes its source-backed infinite penetration/shared-by-type `idStaticNPCHitCooldown=10` facts at the correct per-subupdate damage boundary. Remaining generic `Player.OnHit`/on-kill effects, other child families, unsupported enchantment/status projectile families, status replication and special projectile families remain open.
...
Checkpoint projectile child/gameplay-AI pass: the admitted Confetti Flask projectile branch owns child type 289 creation instead of treating Confetti as a cosmetic/status no-op. Against NPCs, Nano Flask's pinned 1.05 damage boost is applied before Confetti child creation and the child is created before `StrikeNPC`; in PvP, the child is created after `Player.Hurt`/authoritative commit and before projectile-local target immunity/penetration, matching the pinned source order for the supported slice. Star Cannon Star type 955 uses the source-backed `aiStyle 5` gameplay state transition that arms `ai[1]` after leaving solid collision, while Super Star type 728 imports only the gameplay-relevant `AI_151` behavior and deliberately omits alpha/rotation/sound/gore/dust. Natural Falling Star type 12 remains fail-closed as a combat AI profile until authoritative day/remix-world state can reproduce its source kill gate. Super Star's on-NPC-hit `SummonSuperStarSlash` -> type 729 child and the For-the-Worthy reflection branch for types 728/955 are now server-owned; remaining child/on-kill families and broader projectile gameplay-AI families remain open.

Checkpoint live-slot/Star child pass: projectile simulation no longer snapshots the active projectile table for the whole phase. The authoritative loop visits physical slots `0..999` live. Within each slot, every local update becomes generation-safe authoritative state before reflection and NPC/PvP combat; a surviving generation is then reloaded so interaction mutations affect the next local update. This reproduces both key orderings from the pinned server: first-free children in later slots may run during the same global tick, while children placed in earlier slots wait, and `Projectile.Damage()` executes inside the local `extraUpdates` loop. Super Star type 728 uses its vanilla generation-local one-hit-per-NPC immunity and spawns Super Star Slash type 729 after committed NPC hit/penetration side effects with source-backed geometry, 0.75 parent damage, `ai[1]=targetY` and server-owned combat provenance. Type 729 now runs its 20x20, `tileCollide=false`, `extraUpdates=2`, 30-local-update lifetime, infinite penetration and shared-by-type 10-tick NPC immunity through authoritative NPC/PvP collision on all three local updates. Positive intermediate simulation states remain silent to ordinary packet-27 replication; only the final surviving local state publishes the normal update, while terminal/interaction despawns still publish immediately. Prediction-hardening follow-up remains explicit: rejected packet-27 divergence should feed bounded suspicion and large divergence should force an authoritative packet-27 correction resend without trusting any reported client state.

Checkpoint extraUpdate interaction pass: TerrariaServer 1.4.5.8 `Projectile.Update` sets `numUpdates = extraUpdates` and calls `Damage()` inside that loop before the ordinary `timeLeft--`/`Kill()` tail. TerraRuntime now mirrors the gameplay-relevant ordering without emitting one network update per subupdate: a positive intermediate state advances the exact generation's authoritative revision/lifecycle silently, reflection/NPC/PvP interaction runs against that state, any surviving mutation is reloaded before the next subupdate, and ordinary lifetime expiration receives its final interaction before removal. Tile/world/behavior termination remains pre-damage. Regression coverage pins intermediate visibility, interaction-driven velocity mutation, interaction despawn short-circuiting, lifetime-vs-pre-damage termination ordering, and type-729's three committed local positions/lifetimes. Full packet-level/differential coverage for remaining projectile families is still open.
