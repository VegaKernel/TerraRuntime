...
2026-09-20 Mechdusa Probe attachment: AI_005 now applies the live Destroyer hitbox/rotation/velocity and source `26 × ai[3]` orbit with invulnerability. Its timer is three updates per tick, reaches 360, and fires the speed-8, target-velocity-leading pink laser from the pre-attachment snapped source center. Out-of-range Destroyer slots reacquire the first active head; vanished Prime or in-range-reused anchors clear the attachment marker and invulnerability. Focused regressions cover the orbit, state, boundary, projectile and recovery branches. Other combined AI and encounter parity remain open.
