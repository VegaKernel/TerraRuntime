...
- [x] Combat-trusted generations reject owner packet-27 state rewrites (`type`, damage, position/velocity, ownership and related authoritative fields) and packet-29 early termination. The only current packet-27 `ai[]` exception is the explicit controlled-magic intent channel: trusted Magic Missile/Flamelash may submit bounded cursor target `ai[0]/ai[1]`, but the server still owns projectile state and movement.
Checkpoint slice: Magic Missile (16) and Flamelash (34) now establish the first authoritative client-steered projectile channel. Strict spawn validation derives magic damage from server-owned equipment state, validates the exact vanilla launch magnitude, requires zero initial ai state, consumes mana on the server, and generation-scopes combat trust. During channeling, matching owner packet 27 traffic cannot move or accelerate the projectile directly; only `ai[0]/ai[1]` cursor intent is accepted and clamped to the source-backed 1920x1200 player-reachable area. Packet-13 `controlUseItem` and the authoritative held item drive release, after which aiStyle-9 movement is server-simulated. Flamelash's two-hit penetration and 12-tick local NPC immunity are represented. Remaining gaps include vanilla release-time NPC auto-target selection, full mana-cost modifier/refill authority, later controlled families and presentation/localAI-only details.
