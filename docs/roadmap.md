...
- [ ] Buff/debuff application from projectile hits. The admitted PvP slice now owns the complete type-specific `StatusPvP` overlap for its current projectile IDs: Fire Arrow type 2 rolls `On Fire!` 1/3 for 180 base ticks and Poisoned Knife type 54 rolls `Poisoned` 1/2 for 600 base ticks. Server-owned `Player.AddBuff` difficulty duration scaling (Classic 1x / Expert 2x / Master 2.5x), inclusive final active update, negative `lifeRegenCount` accumulation and authoritative HP loss are modeled for `Poisoned`, `On Fire!` and `OnFire3`/Hellfire. Status is applied after the general-immunity gate and before `Player.Hurt`, so a successful dodge retains the debuff exactly as vanilla does. Full `StatusPvP`, flask/frostburn/other attacker enchantments, buff immunity/accessory exceptions, visual/network status publication and remaining DoT modifiers are still open.
...
- [ ] Complete NPC/projectile side effects. Trusted admitted hits now commit NPC damage/death through the existing pipeline and consume source-backed penetration. In PvP, a collided hit that dodges still consumes penetration and starts the projectile-local 40-tick target cooldown, while a globally immune player is skipped before those side effects, matching `Damage_PVP`. Current admitted type-specific status effects and `magmaStone` on-hit state are server-owned; child spawns, generic `Player.OnHit`/on-kill effects, remaining enchantments/status families, status replication and special projectile families remain open.

Checkpoint PvP collision/immunity pass: the strict admitted player-projectile path no longer treats one generic cooldown as all immunity. `Player.Hurt(pvp:true)` general immunity is held for 8 ticks, while each exact projectile generation keeps a generation-safe 40-tick target row matching vanilla `Projectile.playerImmune[target]`. Dodge order is source-matched as Mystic Sash 1/10 -> Black Belt/Master Ninja Gear 1/10 -> Brain of Confusion 1/6 when buff 321 is absent -> active Shadow Dodge. Successful dodge grants the vanilla 80-tick immunity (120 with Cross Necklace `longInvince`), Brain dodge records authoritative buff-321 cooldown for 240 ticks, and Shadow Dodge consumes its authoritative buff state. Dodge consumes projectile penetration/cooldown as vanilla does; pre-existing global immunity does not. Bone Shard type 1124 also observes the pinned `ai[0] >= 15` collision gate. Hallowed-set `onHitDodge` acquisition/cooldown, Shimmer dodge, remaining dodge/avoidance sources and exceptional projectile collision hooks remain open.

Checkpoint PvP status/DoT/on-hit pass: `StatusPvP` is no longer a client-side decoration for the admitted projectile set. Fire Arrow and Poisoned Knife status rolls use server RNG and server-owned lifetimes, and the player owner advances vanilla bad-life-regeneration accumulation into authoritative HP changes. `Fire Gauntlet` now projects its source-backed `magmaStone` state instead of only its +12% melee stats; standalone Magma Stone item 1322 is admitted as the same flag. Direct melee PvP reproduces `StatusToPlayerPvP` magmaStone -> buff 323 with the 1/7 360 / conditional 1/3 120 / otherwise 60 base-tick branch before dodge/Hurt, while admitted melee projectiles (Enchanted Boomerang 6, Waffle 1012, Melee Bone 1111) reproduce `Projectile.StatusPvP` magmaStone -> `On Fire!` with the 1/4 360 / conditional 1/2 240 / otherwise 120 branch. Difficulty extension is applied at the authoritative AddBuff boundary. Packet 50 remains non-provenance. Flask `meleeEnchant`, frostBurn/Frost armor, the rest of `StatusPvP`, generic `Player.OnHit`, Hallowed/Shimmer dodge and exceptional collision hooks remain open.
...
Checkpoint slice: the shared contracts explicitly represent `AttackContext -> AuthoritativeAttackDamage -> TargetMitigation -> FinalDamageToHp`. The authoritative combat snapshot now composes equipment plus generation-safe, tick-owned server-confirmed combat buffs. The modeled combat-modifier subset is Ironskin, Magic Power, Archery, Tipsy, Well Fed / Plenty Satisfied / Exquisitely Stuffed, Weak, Ichor, Broken Armor, Ammo Reservation, Endurance, Rage, Wrath, Shadow Dodge, Brain of Confusion dodge cooldown and the Neutral Hunger/Hunger/Starving states; server-owned PvP status state additionally covers Poisoned, On Fire! and OnFire3/Hellfire DoT. Packet 50 cannot grant these modifiers/statuses by itself. Direct melee/tools (Copper Pickaxe/Axe/Hammer/Broadsword and Muramasa) consume the same snapshot. Equipment projection now covers the main pre-Hardmode metal and class armor families represented in the pinned source, including Pumpkin/Gladiator/Ninja/Fossil/Necro/Shadow/Crimson/Meteor/Jungle/Molten, plus the modeled glove, emblem, scope, quiver, Celestial Stone/Shell, Cobalt Shield, Shark Tooth, Cross Necklace, Black Belt/Master Ninja Gear, Brain of Confusion, Mystic Sash, Magma Stone and Fire Gauntlet families and combat accessory prefixes. Fire Gauntlet/Magma Stone now also project authoritative `magmaStone` on-hit provenance. Unknown active combat equipment still fails closed. PvP target mitigation consumes the equipped+buffed target snapshot and source-backed Classic/Expert/Master player-defense effectiveness plus endurance/no-knockback semantics. Server-side provenance from actual potion/food/NPC/world gameplay sources, Hallowed/Shimmer and other remaining dodge variants, later armor/accessory families and the remaining buff/status catalog are still open; unsupported or unconfirmed combinations must not raise `CombatTrusted` envelopes.
