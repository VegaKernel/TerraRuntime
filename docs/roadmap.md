...
2026-09-15 NPC Start-index extension: the shared vanilla allocator and committed spawn intents now carry a bounded NewNPC.Start index, with ascending inclusion and reverse exclusion verified against25056 additional original decisions. Skeletron hands, Skeletron Prime arms, the Wall of Flesh initial eye/Hungry batch and later Hungry, Plantera's three initial hooks, Golem's detached head, and the Moon Lord shell use their parent slot as in the source; regressions confirm those batches allocate above a root in slot 10 despite earlier free slots. Other producers, especially batched worm chains and remaining boss children, still need individual Start mapping and combined simulation evidence. This supersedes the allocator-API Start limitation in the earlier entry below; it does not close full NPC parity.
