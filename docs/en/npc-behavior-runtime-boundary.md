...
Peer consumers receive a fresh bounded snapshot before each step. The implementation reuses allocated buffers but copies at most `Capacity` squared snapshots per tick (`Capacity <= 256`); admitting larger tables requires replacing this copy boundary with an authoritative read-only live lookup. This establishes NPC traversal semantics, not full vanilla AI or projectile traversal parity.

Committed NPC effects may use an after-spawn phase once allocations and source links have committed; the executor revalidates source generation/revision before invoking it. Exact-generation translations publish `NpcStateCommitKind.ForcedUpdate`. `RuntimeNpcStore.TryUpdate` also accepts an explicit `forceSync` flag for such source-required synchronization. The replication registry bypasses ordinary motion cadence for these commits, preserving packet layout, current baselines and telemetry. This supports Moon Lord's source-ordered core/part relocation.
...
The runtime-owned boss boundary now carries the remaining server-authoritative NPC-side state needed by the late Hardmode/endgame roster instead of leaving the encounter roots attached to metadata-only projectile placeholders. Duke Fishron admits AI 71 Sharkron/Sharkron2 emergence and charge state plus source-owned Sharknado Bolt `385`. Sharkron2 advances its source cosine emergence wave as a position delta, keeps the AI-owned alpha/rotation and collision retirement transitions, and rejects unknown AI71 states before mutation; the projectile runtime carries aiStyle-65 movement and generation-safe on-kill transition into Sharknado `384` or terrain-anchored Cthulunado `386`. Lunatic Cultist admits Ancient Vision/Light/Doom state, ritual Dragon-or-Vision spawning, ritual interruption by the real boss or a struck clone, and the source-owned `464/465/467/468/490/593` attack families.

Empress of Light stages the specialized lasting-rainbow, rainbow-streak, lance and sun-dance projectile families (`872/873/919/923`); daytime rage also promotes the source cadence and `9999` projectile damage rather than relying only on a remembered `ai[3]` flag. In a Remix world, rage instead latches when the first physical Empress slot rises strictly above world surface, and remains active after it returns below. The authoritative world clock also feeds AI-120: a genuinely enraged Empress enters its source retreat at night or at daytime tick `53,400`; state 13 fades by the source five alpha units per tick, returning to attack after 20 safe ticks or retiring at alpha 255. At half health, state 1 requests state 10; only its tick 90 promotes phase through `ai[3]`, relocates the boss above the player, and raises defense by 20 percent. Dash states 8/9 are vulnerable only on ticks 6?40 and use 150 percent contact damage on ticks 41?90. Moon Lord hand/head/True Eye attack clocks follow their pinned 600/1200-tick source sequences and stage Phantasmal eye/sphere/deathray/leech/bolt intents (`452/454/455/456/462`). Moon Lord Hand also owns the pinned tick-292 cross-entity Sphere release: after the exact hand generation commits, only its generation-owned type-454 projectiles are changed to `ai[0] = -1` and source speed 12. The first lethal strike against a hand or head now follows the pinned `NPC.checkDead` survival transition: life is restored, the part becomes invulnerable in `ai[0] = -2`, exactly one True Eye is spawned from that part with the source 1200-tick loop / 588-base / 400-per-existing-eye phase calculation, and the core opens only while both owned hands plus the owned head still exist in their retired state. A retired head also enters `ai[0] = -3` when the core begins its death drama. Core terminal death advances through the authoritative 600-tick lifecycle, and a protected core directly removes itself when an exact retained child slot is absent, inactive or of the wrong type. Cosmetic sound/dust/gore remains outside authority. Remaining Moon Lord parity work is global event/announcement behavior, special projectile-style internals and wider encounter differentials; remaining seed-only Empress conditions also stay explicit.
