...
The runtime-owned boss boundary now carries the remaining server-authoritative NPC-side state needed by the late Hardmode/endgame roster instead of leaving the encounter roots attached to metadata-only projectile placeholders. Duke Fishron admits AI 71 Sharkron/Sharkron2 emergence and charge state plus source-owned Sharknado Bolt `385`; the projectile runtime now carries its aiStyle-65 movement and generation-safe on-kill transition into Sharknado `384` or terrain-anchored Cthulunado `386`. Lunatic Cultist admits Ancient Vision/Light/Doom state, ritual Dragon-or-Vision spawning, ritual interruption by the real boss or a struck clone, and the source-owned `464/465/467/468/490/593` attack families.
