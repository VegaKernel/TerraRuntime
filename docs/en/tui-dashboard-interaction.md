...
`+ Bot` creates a primary-world PlayerBot by default through the primary runtime's real typed bot operations; the shipping composition does not leave the control disabled. Bot rows display the body kind, and double-clicking a bot row opens a typed settings window. PlayerBot fields are limited to automatic/bow/gun/melee weapon policy, mobility accessories, target and Idle/Follow/Guard mode. Pickup, healing and supported buffs are always enabled. Clothing/hair and admitted armor are generated per bot and no longer have UI presets. NpcBot shows only one entry per applicable source-verified NPC type, target and movement mode; player-only loadout/mobility controls are hidden.

PlayerBot reuses the normal server-player/projectile/world-item authorities. Its automatic loadout visibly selects between Copper Broadsword, Wooden Bow and Musket and publishes the selected slot plus use-item control. Ranged aim uses the admitted projectile's source-backed ammo speed and motion, predicts a moving target, and rejects attacks whose tile-aware line or simulated trajectory is blocked by tiles or liquid. Movement publishes vanilla direction controls, auto-jumps low obstacles and can hold source-backed wing flight to ascend past an airborne forward obstruction. Different bots receive separated escort offsets and movement phases instead of copying the same destination. The enabled mobility loadout occupies the five ordinary accessory slots with Fishron Wings, Soaring Insignia, Terraspark Boots, Magiluminescence and Master Ninja Gear; the admitted horizontal physics applies the verified Terraspark and grounded Magiluminescence run-speed slice without inventing dash behavior.

NpcBot is a real authoritative hostile-NPC presentation/motion actor; available types are restricted to verified ground-fighter, AI_002 flying-eye, AI_005 flyer and ordinary pre-wander AI_014 bat motion families. Follow/Guard uses a separated escort point rather than the player's center, coordinates remain private actor intent, and replicated vanilla NPC target stays `255`. This separation is required because an unmodified client derives contact damage from the NPC type and cannot receive the server's per-instance zero-damage override. The authoritative body remains zero-contact and invulnerable until bot-specific death/drop semantics exist; unsupported NPC combat semantics stay fail-closed.
