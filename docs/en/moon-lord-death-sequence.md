...
Additional regressions retain the same head, local state, shared random stream and created projectiles over two full attack cycles ($2400\,\text{ticks}$). The stationary original-server trace matches every update, including exact projectile state and the final random draw; its fixture SHA256 is `405a45608d1796c0b3e8be07d9d04433399dcd0ec102893967bbd12a57cd6062`. A second trace moves the player by `(3.25, -1.75)` pixels per tick and supplies that velocity to every target refresh; it also matches state, active projectiles and final RNG with fixture SHA256 `92e40bc2e5cc69f9333debb2975c29c5277b4d5ef09f832efaa6d4d3ad7c6ffa`. Both traces run head AI only and do not advance projectile AI or outer NPC physics.
