...
### NPC steady-state cadence

`RuntimeNpcReplicationRegistry` is a transport projection fed only by the authoritative game loop. It sends packet `23` immediately for spawn, despawn and committed HP/max-HP changes. Ordinary changing motion/AI snapshots are sampled at the default TerrariaServer `1.4.5.8` `Main.npcStreamSpeed` interval of $30\ \text{ticks}$, rather than being emitted for every simulation revision. The latest snapshot is still retained as the late-join spawn baseline.
...
Recipient selection is the source rule, not a runtime approximation. TerrariaServer 1.4.5.8 distributes a tick's liquid change set into $200 	imes 150$ network chunks and broadcasts each chunk only to clients whose `TileSections` entry for that chunk is set: `NetLiquidModule.ChunkChanges.BroadcastingCondition` reads `Netplay.Clients[clientIndex].TileSections[ChunkX, ChunkY]`, and the legacy `NetMessage.sendWater` path applies the identical `TileSections[x / 200, y / 150]` test. The gate is therefore section ownership, never distance. TerraRuntime evaluates the same predicate against `PlayerSectionStreamingState`, the per-connection table of sections already transferred by packet `10`.

The test is applied twice, for different reasons. A simulated cell that no playing connection could receive never enters the retention map, so world-wide simulation noise cannot occupy the bounded pending set or starve the cells a client can actually see. The test is repeated when a frame drains, because a connection can cross a section boundary between admission and drain. Frames discarded by either check are counted separately as section-filtered liquid updates. A connection with no section table - the bootstrap-only endpoints used by tests and the terminal harness - keeps the unconditional behaviour.

The authoritative liquid state and its update ordering are unchanged. Network detail exposes emitted, pending, coalesced, evicted and section-filtered liquid updates; the rolling message table remains the on-wire rate authority.
