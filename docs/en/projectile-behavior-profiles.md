...
| `Bomb` | Bomb 28, Dynamite 29, launcher types 133..144 and verified Mini Nuke types 793..810 implemented | exact aiStyle-16 ordinary-fuse/grenade/mine/straight-rocket motion; trusted termination owns the source-backed combat explosion and exact admitted terrain destruction |
| `CelebrationRocket` | holder 714 and child rockets 715..718 implemented | holder stays presentation-only; exact volley provenance and aiStyle-147 child motion are server-validated; only source-backed terrain-destructive variants receive terrain authority |
...
- explicit hostile boss/NPC families for straight beams, Plantera seeds, Golem fireball, Duke Fishron tornado chains and Cultist Ice Mist/Lightning/Fireball slices, including their non-generic gravity/collision rules;
- post-commit live-child staging for aiStyle-64 tornado segments/Sharkron children, aiStyle-86 Ice Mist children and aiStyle-88 Lightning Arc children, with exact projectile/NPC generation provenance revalidation before publication;
...
The currently admitted live-child slice is deliberately post-commit. A Sharknado/Cthulunado transition from `ai[0] = 2` to `1` may stage its next segment and the source-interval Sharkron/Sharkron2 spawn only after the exact parent projectile generation commits; the authority then revalidates the exact originating `NpcHandle` generation. Cultist Ice Mist uses the same boundary for the emitter's 30-update child cadence. Cultist Lightning Orb uses it at updates 30/60/90/120/150, then revalidates its NPC provenance before selecting up to five line-of-sight players in physical slot order and publishing seeded Arc children. This prevents reused projectile or NPC slots from inheriting stale child work. Exact same-pass physical-slot update ordering for newly allocated children remains open.
...
Combat trust is generation-scoped. Server-created generations may be trusted directly, while a client packet-27 generation crosses the same boundary only when strict source-backed provenance succeeds. The admitted client paths currently cover ordinary early bows/arrows, basic guns with Musket Ball/Silver Bullet, Grenade Launcher/Rocket Launcher/Proximity Mine Launcher with Rocket I-IV and Mini Nuke I/II, Bomb/Dynamite, Celebration Mk2, selected-stack Shuriken/Bone/Throwing Knife/Poisoned Knife/Rotten Egg/Star Anise/Bone Dagger, and prefix-free Magic Missile/Flamelash/Rainbow Rod channeled magic. The rocket-ammo path reproduces vanilla's base-projectile-plus-ammo-offset transform instead of accepting the packet projectile type as truth. Celebration Mk2 keeps holder 714 untrusted and admits children 715..718 only through the source-backed seven-pattern volley cadence/capacity, launch speed and one-ammo-per-volley boundary. Provenance validates selected weapon, first compatible ammo where applicable, projectile transform/type, server-calculated damage/knockback, launch-speed magnitude, admitted initial `ai[]`, spawn distance and source-backed use cadence; ammo/throwable consumption and admitted magic mana consumption are committed server-side. Unsupported sources remain compatibility state and cannot damage authoritative entities. Once an ordinary generation is trusted, owner packet 27/29 traffic can neither rewrite position/velocity/AI or identity/damage fields nor terminate it early. Magic Missile/Flamelash/Rainbow Rod are the deliberate exception for **intent, not state**: packet 27 may update only `ai[0]/ai[1]` as the requested cursor target after generation/type/damage ownership checks; the server clamps that point through the source-backed 1920x1200 player-reachable view rectangle and ignores client position/velocity. Packet-13 `controlUseItem` plus the authoritative selected item releases the projectile. The server then performs the source-backed 800-pixel nearest-NPC line-of-sight lookup in physical slot order for the currently modeled chaseable candidate set and re-steers toward the authoritative NPC center; without a target it normalizes toward the 32 px/tick released speed. Rainbow Rod additionally keeps its channelled tile-collision 10% component damping instead of being killed by that contact.

For combat-trusted player projectiles, the current hit passes admit only behavior profiles whose source-backed collision/penetration semantics are represented. NPC hits select live generation-safe targets by source-backed AABB geometry, use the ordinary shared owner/NPC immunity baseline for admitted multi-hit families, use permanent projectile-local NPC immunity for grenade variants 133/136/139/142, and use Flamelash/Rainbow Rod's source-backed 12-tick projectile-local NPC cooldown. That local-immunity state is one generation-safe registry shared by collision and controlled-magic target lookup, matching the source fact that both `Damage_PVE` and `FindTargetWithLineOfSight` consult `Projectile.localNPCImmunity`. After a committed released Flamelash/Rainbow Rod NPC hit, the source-backed post-hit `ai[1] = -1` reset is committed before the next AI_009 step, so reacquisition skips still-immune exact NPC generations and can select them again at the 12-tick boundary. Hits resolve damage variance/crit/armor penetration from server-owned combat state, commit through the existing NPC pipeline, and consume penetration only after a committed hit. A separate generation-safe PvP pass selects hostile legal players, rejects same-team targets, resolves PvP damage without packet 117, applies the vanilla 40-tick projectile/player immunity baseline, and consumes the same source-backed penetration state. At the source `StatusPvP` ordering point before `Player.Hurt`, the admitted type-specific rules can also publish targeted packet `55`: Fire Arrow `2` rolls `On Fire!` `24` for 180 ticks at `1/3`, Flamelash `34` rolls `On Fire!` for 240 ticks at `1/2`, and Poisoned Knife `54` rolls `Poisoned` `20` for 600 ticks at `1/2`. Because vanilla calls `StatusPvP` before `Hurt`, Creative GodMode may avoid HP loss while the status proc still occurs. Delivery is exact-target-generation only; it is not observer buff replication. Positive penetration counts down, the final hit despawns the exact generation, and infinite penetration remains active.

World motion already owns tile collision, liquid contact, world bounds and source-backed lifetime. The server-owned hostile simulation slice additionally executes the source-backed straight AI_001 beams used by Wall of Flesh, Probe, Retinazer and Golem (83/84/100/259), Plantera Seed/Poison Seed (275/276), Plantera Thorn Ball (277), Golem Fireball (258), Skeletron Prime Bomb (102), Duke Fishron Sharknado/Sharknado Bolt/Cthulunado (384/385/386), Phantasmal Eye/Sphere/Deathray (452/454/455), Phantasmal Bolt (462), Cultist Ice Mist/Lightning Orb/Lightning Arc/Fireball/Fireball Clone (464/465/466/467/468), Hallow Boss Lasting Rainbow/Rainbow Streak/Death Aurora (872/873/874), Fairy Queen Lance/Sun Dance (919/923), Queen Slime Smash (922), Ancient Doom projectile (593), and Queen Slime gel (926). Runtime lifecycle owns vanilla `Projectile.localAI[0..2]`, including the source integer-resized Sharknado/Cthulunado hitboxes, Queen Slime Smash's expanding square, Cultist Lightning Arc's seeded eighth-subupdate turns and tile-stop state, Phantasmal phases/beam state and Empress warm-up timers. A generation-scoped 20-position registry reproduces the Arc's exceptional trail collision while its stopped tail collapses. Moon Lord Hand's source tick-292 transition now releases only exact-generation-owned Sphere 454 projectiles by setting `ai[0] = -1` and speed 12 after the NPC state commit succeeds. Phantasmal Eye/Sphere Kill() damage is replayed as source 144x144/208x208 hostile AABBs with original knockback and exact NPC provenance. Sharknado Bolt aiStyle 65 now owns source wave/homing/wet-kill behavior, and its committed Kill() stages Sharknado 384 or terrain-anchored Cthulunado 386 with classic/expert damage while preserving the originating `NpcHandle` generation. World-bounds removal does not create Kill() side effects. Presentation-only dust/sound/lighting is not promoted into authoritative gameplay state. Bomb/Dynamite, launcher 133..144 and admitted Mini Nuke projectiles own their exact aiStyle-16 motion; Celebration children own the necessary aiStyle-147 motion/target slice. A trusted terrain-destructive termination is queued into `WorldTileAuthority`, which applies the exact strict-radius `Projectile.ExplodeTiles` gate, protected tile/wall rules, drops, liquid wake and replication. Matching owner packet-17 traffic is consumed only as bounded convergence echo; an untrusted projectile never receives terrain authority.

The runtime still fails closed on the important missing pieces: full weapon/ammo/provenance coverage beyond the admitted families, other controlled/channeled families such as legacy Flying Knife, yoyos and holdouts, complete mana modifier/refill authority, unsupported multishot/special spawn parameters and explosive projectile families, the remaining vanilla projectile AI families, remaining local/static immunity variants, exceptional hitboxes/target rules, explosive self-hurt/owner-hit semantics, remaining projectile-applied buff/debuff effects (notably hostile PvE status and equipment/enchantment-derived PvP status), unsupported object-destruction side effects, and exact same-pass child/on-hit spawn ordering. Packet `50` now provides only the source-correct generation-scoped buff-type presentation mirror; vanilla dedicated server skips hostile `Damage_EVP`, and packet `50` carries no duration, so that mirror is deliberately not promoted into combat authority. Client packet-27 generations outside the strict source-backed catalog therefore remain synchronization/diagnostic state rather than authoritative combat or terrain sources.
