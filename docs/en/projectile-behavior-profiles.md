...
| `ControlledMagicMissile` | Magic Missile 16 / Flamelash 34 implemented | aiStyle-9 steering is server-simulated; trusted packet 27 contributes bounded cursor intent only, while position/velocity/damage remain authoritative; release follows server-owned `controlUseItem`/held-item state |
...
- the player-owned aiStyle-9 controlled-magic profile for Magic Missile/Flamelash, including its server-only ownership gate;
