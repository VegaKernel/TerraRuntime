...
Moon events now take the source `NPC.Spawner.SpawnAnNPC` selection table for waves one through five after the source Dungeon and Meteor priorities. The server preserves source random-draw order and active-type caps, uses admitted ordinary fighters when selected, and simulates Snow type 341 with its source AI_025 wait/jump cycle. Other special-AI selections remain fail-closed rather than becoming ordinary hostiles; later waves remain open.
