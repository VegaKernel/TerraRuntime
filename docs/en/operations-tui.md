...
The roster action row is **`+ Sandbox`**, then **`+ Bot`**. Bots are now PlayerBot-only: hostile NpcBot/defender creation and presets have been removed. The normal NPC actor, interaction and shop foundation remains available separately. Double-click opens weapon policy, flight, target, Idle/Follow/Guard and GodMode settings. Bots start with 500/500 HP and 200/200 mana, unique generated names, coordinated randomized functional/vanity loadouts and stocked healing/mana potions. Pickup, healing and supported Archery/Wrath use are automatic; consumed items come from authoritative inventory. Potion use avoids trivial overhealing, and healing plus mana in one tick preserves both updated vitals. Admitted NPC contact, hostile projectiles and their termination explosions use the existing authoritative damage pipeline; lethal damage produces a real dead state and packet 118. Unsupported combat/equipment effects are not implied by the loadout.

PlayerBot reuses the existing server-player, projectile and world-item authorities. Automatic weapon selection uses supported Muramasa, Platinum Bow and gun loadouts with Unholy Arrows/Silver Bullets; held slots and item-use controls are replicated. Aim uses actual weapon/ammo/accessory speed, source projectile substeps and tile-aware line/trajectory checks. Guard keeps a generation-safe target lock, approaches effective range, holds a clear firing position and uses a bounded, stable reposition destination to recover a blocked shot. Flight can climb that obstruction; Follow retains separated formations and walking/flight traversal. Catch-up recovery shows an inventory-owned Magic Mirror (item 50, 90-tick use), relocates at tick 45 and resumes weapon use afterward. Packet 12 RecallFromItem supplies the observer's recall floor. Following a player rather than recalling to a saved bed is explicitly bot policy, not a new vanilla mirror rule. Full pathfinding, every weapon/accessory effect and live-client visual parity remain outside this tested slice.

The tester follow-up fixes protocol-326 remote item presentation: packet `13` now carries normal gravity and the successful-use bit required by remote `Player.ItemCheck`; aimed shots also publish packet `41` with authoritative rotation/animation. PvP melee claims, trusted player projectiles and termination explosions now include server-owned bot targets through the existing damage/mitigation boundary, with hostility/team/generation checks and GodMode preserved. Lethal PvP emits packet `118`. Bullet `14`, Silver Bullet `981`, Green Laser `20` and Jester's Arrow `5` keep the source AI_001 no-gravity timer behavior; prediction uses the same distinction. Open bot settings refresh live PvP status. Losing the exact target generation clears the target and selects Idle, without name-based reconnect attachment. Follow contracts a formation offset that lands inside solid terrain. World replacement deactivates all remote player slots before the destination baseline, excluding the connection's own slot and sentinel `255`; primary bots remain in primary. Three repeated transfers are covered, but this does not establish the cause of the tester's Lost connection or replace live-client visual acceptance.
...
Repeated telemetry snapshots do not append artificial zero samples. Unchanged world-list text retains its ListView source, and refresh invalidates only changed child views, not the whole dashboard. A framebuffer regression checks that unchanged network data is not redrawn and new samples still draw.
