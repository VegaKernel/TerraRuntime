...
The roster action row is exactly **`+ Sandbox`** followed by **`+ Bot`**. `+ Sandbox` opens the typed sandbox creation form. `+ Bot` accepts through the shipped dashboard command binding and creates a primary-world PlayerBot by default; double-clicking a bot row opens its typed settings window. Bot settings expose PlayerBot/NpcBot body, one deduplicated source-verified NPC type, weapon policy, mobility accessories, target and Idle/Follow/Guard mode. Pickup, healing and supported combat buffs are always enabled PlayerBot capabilities rather than switches. Clothing, hair and the admitted armor set are generated per bot instead of selected from UI presets. PlayerBot reuses existing server-player/projectile/world-item authorities; NpcBot is a real authoritative NPC presentation/motion actor. PlayerBot normally walks at the protected player's ground level. Fishron Wings and the admitted mobility accessories remain equipped as traversal capabilities: the controller holds ascent only when the player is materially above or a solid line-of-sight obstruction blocks the route, then targets a visibly higher formation point instead of permanent low hover. Its supported NPC types are restricted to source-backed controlled-motion families (ground fighters, AI_002 flying eyes, AI_005 flyers and ordinary pre-wander AI_014 bats). NpcBot escort destinations use separated per-bot formation offsets and retain replicated vanilla target `255`; the bot body is also server-side zero-contact plus invulnerable so it cannot enter ordinary NPC death/loot/progression paths. Supported PlayerBot Archery/Wrath potion effects are trusted server-side combat state; they are not broadcast to unrelated observers as packet `55`, because vanilla 1.4.5.8 applies that PvP-buff packet only to the encoded local player. Unsupported NPC combat semantics remain fail-closed rather than being approximated.
