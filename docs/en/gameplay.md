...
PlayerBot uses a [typed task/action layer](runtime-bot-architecture.md). `Mining` now autonomously finds and acquires the selected ordinary ore underground; `Collect` and `ReturnToPlayer` retain their existing semantics. Follow/Guard assistance, physics, combat, pickup and consumables retain their authorities. Mining is limited to eight ordinary ores and local Dirt/Stone access, not global exploration, building or crafting parity.

### Live falling blocks (2026-09-09)

Unsupported sand, ebonsand, pearlsand, crimsand, silt, slush and shell piles detach through `WorldTileAuthority`, become exact-generation server-owned AI `10` projectiles, and land through the same mutation/item authorities. The initial server velocity, first collision-disabled update, gravity, wet movement, platform/half-block landing, protected supports and conveyor/item conversion follow the verified `1.4.5.8` slice. Normal projectile combat applies player armor, immunity, GodMode and death (`packet 118`) without inventing an NPC source; admitted hostile NPC targets use the existing non-player damage/loot pipeline. Client-created projectile types do not grant terrain or damage authority.

Work is bounded per world: $256\,\text{checks/tick}$, $16\,\text{spawns/tick}$, $128\,\text{active-or-pending falls}$, and $4096\,\text{queued wakeups}$. Normal tile mutations wake neighbours immediately. Initial/overflow discovery is a resumable scan; overload delays settling rather than doing an unbounded full-world pass. A full projectile pool leaves the source tile intact. A full item pool retains the landing event until capacity is available. These limits are runtime scheduling policy, not vanilla timing parity. Stress coverage checks conservation across a $160\,\text{block}$ collapse and pool pressure; official AI/movement fixtures cover seven types in dry/wet/platform/half-block scenes.

Coin piles, sandgun/player-owned channels, AI tile-cut decoration side effects and complete town-NPC interactions are not admitted by this slice. Unknown tile-cutting intersections remain fail-closed. In-flight projectiles are not newly persisted; existing world-save semantics and format are unchanged. Like vanilla projectile replacement/edge deactivation, non-Kill removal does not invent a terrain restoration. Whole falling-block/gameplay parity is not claimed.

Local validation: Release $5555/5555$ tests, zero build warnings/errors, Windows NativeAOT and six smoke modes passed. An isolated $160\,\text{block}$ collapse over $300\,\text{ticks}$ after the initial spawn took $58.441\,\mathrm{ms}$ wall time and allocated $102688\,\mathrm{bytes}$ on the writer thread including assertions. This measures the fixture, not a production latency guarantee or before/after optimization. Linux NativeAOT and official GUI client acceptance remain unverified.

### Bot traversal correction (2026-09-08)

Underground escort continuation: bounded full-body search also handles multi-bend cave corridors. Invalid formation points contract to the followed player's position. Mirror landing searches nearby dry, clear positions **after** `Spawn_SetPosition` floor-anchor quantization; an enclosed destination cancels the teleport, never excavates or embeds the actor. This remains local navigation, not complete cave pathfinding.

Bots carry Vortex Pickaxe `2776` in hotbar slot6. Follow/Guard assistance requires a recent (120 ticks), successful authoritative simple-tile break by the exact followed player generation in the same world. Both players and the candidate tile must be below verified `WorldSurfaceTiles`; missing metadata, surface mining and intermediate `fail=1` packets grant nothing. This assistance only widens obstructing nearby Dirt/Stone passages, with short reach, no walls/liquids/wired or neighbouring special tiles. It reuses `WorldTileAuthority` pick requirements, mutation, reserved drop transaction and replication, with12-tick animation cadence. Ores, structures, furniture, temple/dungeon demolition and autonomous goals are excluded from this assistance; the separate Mining mode is documented above. Source item facts are pick225/useTime6/useAnimation12; policy deliberately waits the full animation without guessing equipment mining-speed modifiers.

Guards of the same player use soft target assignment: distribute non-immediate threats, preserve target locks, allow shared focus on a sole enemy, and prioritize threats within80 pixels of the protected player. Melee pairs approach distinct offsets; ranged guards reposition around allied firing-lane occupancy, with deterministic yielding at shared spawn positions. Separate protected-player handles never share assignments. This is server bot policy, not a claim of vanilla NPC AI parity or general squad tactics.

Four functional slots now hold Fishron Wings, Soaring Insignia, Terraspark Boots and Magiluminescence; the fifth remains kit-specific. Disabling flight retains grounded accessory movement. Shared physics requires functional wings, not an inventory/vanity item or policy flag alone. Active WingMovement skips gravity. Fishron airborne speed8/acceleration2, Insignia move-speed+.075/acceleration1.75/jump-speed+1.8 and post-decrement flight refresh follow1.4.5.8. Without Insignia the admitted budget is180 updates; airborne jump release does not refill it. Rocket-to-wing fuel conversion, other wings, dashes and compound movement effects remain open.

Bounded bot policy checks the full20x42 body along multi-leg overflight/side-exit routes and retains a short-lived waypoint. It submits normal movement intent; collision, velocity, combat and mirror recovery remain with existing authorities. This is local obstacle avoidance, not general pathfinding or official-client visual acceptance. Regressions include actual100-tick upward pursuit, equipment admission and wall/overhang/enclosure cases.

Acceptance tooling correction (2026-09-08): the live Dirt probe uses separate Dirt/pickaxe hotbar slots, waits for the real inventory relay, and verifies packet-13 selection and a position inside the ordinary world border near the edited tile. Identical movement is legitimately deduplicated, so replacing an item in the same slot is not a movement acknowledgement. Production mining admission remains unchanged. The Moon Lord source gate recognizes both XNA-resolved constructors and ILSpy's unresolved-XNA by-reference constructor form, while pinning the temporary, operands and interpolation amount. Changed damage/AI constants are not accepted as decompiler variation.
...
Full pickup uses **packet 151**, not a zero-stack packet 21 on the normal vanilla wire: `NetMessage.SendData(21)` rewrites an empty item to the compact removal frame. Inbound 151 and the source-supported empty-21 form reach the same exact-generation, reservation-owner removal boundary. Ordinary removal and instanced lease expiry both encode the source five-byte packet 151. An unowned/reused item or inactive leased slot cannot be removed by an arbitrary client; inbound packet 22 remains non-authoritative. Ignoring 151 formerly left client-collected items in the server's bounded pool, eventually refusing every drop-producing tile break. Literal-wire regressions exercise 900 mining/tool-switch/pickup cycles with both empty and full main inventories, without raising pool or network limits. Packet 151 retains the existing aggregate flood ceiling; copying the packet-21 limit would reject a legal full-pool pickup burst.

World-item merging/overflow eviction, special magnets, Void Bag routing, full item physics and remaining `ItemSpace`/`CanPullItem` branches remain incomplete. A genuinely full pool still rejects a drop-producing break before tile mutation instead of losing the drop. The catalog additionally admits source-verified Nebula/Solar Flare/Stardust pickaxes `2781/2786/3466`, each with pick power `225`. This fixes demonstrated paths; the tester's exact transient incident cannot be fully reconstructed without a log or packet capture.
...
Duke Fishron's ordinary AI69 now consumes verified world width/surface through the existing NPC authority. Ocean checks use the player's top-left (strict800/6400/surface/right-edge comparisons). Enrage changes hover to10 ticks, adds6 to dash speed, replaces bubble specials, overrides damage/defense and marks the Cthulhunado bolt; returning to the ocean restores normal phase behavior. Ordinary Classic/Expert/Master defDamage is100/140/210 before phase/enrage. Missing world bounds refuse the root step/projectile plan. AI69 now also refreshes a distant retained target before the 5600-pixel retreat check, lowers vertical velocity, caps lifetime and resets the phase clock, then continues through initialization, rotation and the selected phase in that same update. The root rotation follows TerrariaServer's pre-dispatch shortest-path turn and phase-specific rate. Duke targeting uses TerrariaServer's mounted-player geometry: normal mount height boosts plus Rat, Bat and Pixie overrides feed target centers, ocean gates and overlap checks. Duke spawn defaults also preserve source Classic/Expert/Master life and damage arithmetic, multiplayer balance, Good World's 1.3 scale/hitbox and Celebration Mk10's half-scale hitbox; Good World correctly takes precedence if both seed flags are present, and AI69 uses that physical Duke body for target range, centers and phase-three relocation. The runtime keeps source wind target separate from current wind, applies the rain-scaled easing term and broadcasts the live target in packet 7. Its server weather slice now also runs once per day-rate unit, freezes with time, honours Freeze Wind after easing, uses the source `windCounter`/`extremeWindCounter` target transitions and caps pre-120-life worlds at absolute target `.35`. The same owner now preserves `raining`, `rainTime` and `maxRaining`: an eligible 120-life player can start the source-shaped ordinary interval, its duration/strength draws and in-progress re-rolls follow the verified branches, Freeze Rain pauses it, packet 7 receives the live rain strength, checkpoints restore it, and town shelter scheduling consumes the live flag. Coin Rain, ladybug boost, Lantern Night, forever-rain and full shared-RNG integration remain open. These source-pinned slices do not complete live wind/weather, other phase details or official-client encounter acceptance.

Empress of Light's admitted `AI_120` planner retains the source preparation retreat on target loss, half-life phase transition, dash preparation/window, alpha and local synchronization counters, and the Rainbow Streak random bearing. Its state-4 Fairy Queen Lance ring is source-shaped: Classic uses four $300\,\text{pixel}$ anchors with the horizontal `0.5f` direction correction; Expert or daytime rage uses five $450\,\text{pixel}$ anchors. The plan refuses targets beyond $2400\,\text{pixels}$, reverses an anchor when the normalized target velocity points along it (including the source `UnitY` fallback for stationary targets), predicts target motion for $90\,\text{ticks}$, offsets the spawn by $30\,\text{ticks}$ of motion, and stores the resulting aim/phase state in the projectile intent. States 2, 4, 11 and 12 also reproduce the source extra-player volley: candidates must have interaction credit for this NPC, fall into the current modulo-three rotation, be active and non-dead, and be within $2400\,\text{pixels}$; the primary target is always skipped. Presentation, complete difficulty branches and full encounter parity are open.
...
### Plantera and Golem death rewards

Plantera and Golem now use the existing authoritative death/interaction/item pipeline for their NPC-specific Classic, Expert and Master rules. Classic Plantera first drops Grenade Launcher and 50..150 Rocket I on the first kill, then mask/key/optional rewards; later kills choose among eight source rules. Loaded `DownedPlantera` and current-world progression both suppress the first-kill branch. Unknown initial world facts withhold this Classic reward slice instead of assuming a fresh world. Expert receives its addressed Boss Bag, not an extra world Temple Key.

Golem preserves mask, independent Picksaw and 1.4.5.8 Mobius Strip chances, seven nested reward options, Stynger ammunition chaining and 4..8 Beetle Husks. Master relic/pet and trophy rules remain source-ordered. Packet 90 bags retain the existing lease/recipient path; clientless bots and stale-generation connections cannot redirect a bag to observers. Duplicate death requests produce no additional rewards.

The 35 item definitions admit world-drop dimensions and natural-prefix families only. Independent official-server comparisons match 3500 item/seed prefix outcomes and subsequent RNG draws; Venus Magnum and Heat Ray retain their item-specific rounding exclusions. Dropping these items does not admit unverified weapon/tool/accessory use. Temple Key packet 52 action 2 now enters the authoritative tile/inventory path: it consumes the first key 1141 in slots 0..57, unlocks the validated three-row temple-door footprint, echoes packet 52 to peers and sends the source 2x2 tile square. Packet 19 actions 0 and 1 also enter tile authority: opening uses the source-shaped 1x3-to-2x3 transform, while the forced close validates the open 2x3 footprint and preserves the source's three ordered frame-column random draws; both relay only to peers. Its forced tall-gate actions 4 and 5 similarly validate the full 1x5 object and switch only its type before peer-only relay. Packet 19 action 2 now resolves the closed type-387 footprint, reserves both supported destination drops before mutation, removes admissible `tileCut`/drip cells through authority, then performs the source type shift and peer-only relay. Destinations whose full `KillTile` drop path is not yet modelled are rejected before any tile changes. Void Bag fallback, the remaining trapdoor `KillTile` families, global luck/coin/heart rules and bag contents remain progression debts. Neither these rewards nor the existing boss AI slices prove a complete vanilla playthrough.

The new evaluators roll the trophy before other NPC-specific rewards: the official database registers trophies before boss tables and preserves insertion order. A direct official `Populate/GetRulesForNPCID` probe confirms this. Older trophy-last boss evaluators remain a separate confirmed ordering debt; their existing tests alone do not prove vanilla RNG parity.
...
Three further `Liquid.Update` mechanics are now authoritative, and together they are what makes a world go quiet rather than merely settle on average.

The three-cell averaging branch carries the one random draw in the whole method: `if (num == 254f && WorldGen.genRand.Next(30) == 0) num = 255f`. It is the only route out of a sealed pocket whose water averages to exactly 254, and the cell above such a pocket keeps handing down a unit through the deliberate full-source exception, so a runtime without the draw leaves that pocket flipping between 254 and 255 for the lifetime of the world. The draw crosses `IVanillaLiquidRandom1458`, so tests pin both outcomes.

Each averaging width also has its own write and wake-up conditions, and the widths do not agree. The seven- and five-cell branches write a neighbour that differs and wake it, then wake every neighbour again when the source's own pre-update amount differs from the new level. The four-cell branches fold both terms into one condition that gates the write as well. The three-cell branch works strictly on the neighbour's own difference. The two-cell branches write on the neighbour's difference but wake only on the source's, which is what lets a settled pair stay settled: a retired `255` pushed back down to a `254` average is corrected without being revived. Horizontal averaging never wakes the rows above or below at all; the only vertical wake-ups belong to the downward-flow step and to the kill tail's own `AddWater(x, y - 1)`.

`Tile.skipLiquid` is also reproduced. A downward transfer marks both of its cells, and the ordinary `Liquid.UpdateLiquid` loop then passes over a marked cell once - unmarking it and leaving its delay and kill state untouched - instead of updating it. The quick-fall loop always updates and clears the mark afterwards, and a successful `Liquid.AddWater` clears it as it takes a fresh slot. The flag is transient simulation state that never reaches the saved world, so it lives with the delay/kill work state rather than on the tile.

The material dispatch at the head of the method is nested rather than flat, and the nesting is now exact: a cell runs only its own `LiquidCheck`, and for each material it is not - in the source's lava, honey, shimmer order - it instead wakes the adjacent cells that are. Lava therefore wakes nothing, honey wakes adjacent lava, shimmer wakes adjacent lava and honey, and water wakes all three.

Measured against the pinned official assembly on a region lifted out of a generated large world, the two engines now agree on convergence: official `Liquid.UpdateLiquid` settles it in $359$ ticks and the runtime in $386$. A generated 8400x2400 world reaches zero active and zero buffered liquid cells and stays there, where before it held tens of thousands of permanently oscillating cells. Two residual divergences are known and not closed: the runtime gains about $0.8\%$ liquid mass over that settling window where the official engine loses about $0.04\%$, both through vanilla's own non-conservative full-source transfer, and `Liquid.stuckCount`/`StartPanic` forced settling is still absent.
