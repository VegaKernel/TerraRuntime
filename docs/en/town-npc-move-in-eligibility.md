...
On the source cadence, `RuntimeTownNpcMoveInCoordinator1458` evaluates the active authoritative player/inventory state, applies the room-aware source selection above, allocates a generation-safe NPC slot when a valid room exists, commits the resident into `RuntimeTownNpcStateStore`, and publishes the NPC spawn plus packet 56 identity, packet 60 home state, and packet 82 localized arrival announcement through the existing NPC replication owner. Runtime-created residents, including their generated given name and variation, are included in subsequent `.wld` NPC/TownRoom save snapshots.
