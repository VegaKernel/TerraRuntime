...
- `VanillaGroundFighterNpcBehaviorStrategy` owns Fighter-family target prepass, overlap semantics, day/surface pursuit policy and the verified `VanillaZombieMotion` compatibility transition. `VanillaGroundFighterNpcCatalog` now pins SetDefaults plus AI_003 movement profiles for Zombie, Skeleton and sixteen additional hostile fighters: Goblin Peon/Thief/Warrior/Scout, Angry Bones, Doctor Bones, The Groom, Armored Skeleton, Bald Zombie, Zombie Eskimo, Undead Viking, Pincushion/Slimed/Swamp/Twiggy/Female Zombie. The profile records each source-backed maximum horizontal speed and whether the source applies the `(1 + (1 - scale))` speed adjustment instead of assuming that every `aiStyle = 3` type shares Zombie semantics.
- Angry Bones and Armored Skeleton additionally opt into the pinned grounded close-range lunge (`|dx| < 100`, `|dy| < 50`, movement toward the target, doubled X clamped to `±3`, Y set to `-4`). The world-motion layer also forwards the concrete NPC type into the AI_003 door-pressure policy, so type-specific pressure/reset rules are no longer silently evaluated as Zombie. Goblin Peon type `26` now owns its distinct authoritative destruction branch: complete closed-door and tall-gate objects are removed, locked Dungeon doors remain protected, the source style item is dropped and the committed break is replicated as packet 17. Ordinary opens remain packet-19 mutations, with tall-gate actor occupancy evaluated from live players and NPCs in production composition.
- `VanillaBatNpcBehaviorStrategy` owns the ordinary AI_014 bat slice for Cave, Jungle, Hell, Giant, Illuminant, Ice and Lava Bat, Giant Flying Fox and Spore Bat. Their pinned definitions and source-ordered motion cover collision rebound, closest-target refresh, the Hellbat correction constants, wet escape and the pursuit/wander clocks; runtime targeting supplies live wet state and line of sight. The Vampire pair also has its source distance, lower-edge and line-of-sight transition from flying type 158 to humanoid type 159, and its 300-pixel return transition. Both transformations preserve the lower edge, scale life proportionally, reset AI-local defaults and retarget. Projectile-shooting AI_014 variants remain fail-closed.
- The same explicit AI_014 strategy now admits Slimer and Queen Slime's Purple Slime minion. Slimer takes the single generic acceleration pass while the minion uses its dedicated `0.35/0.30`, `6/5` steering profile; neither is accidentally given the bats' second acceleration/wet branch.
- `VanillaFishNpcBehaviorStrategy` owns all thirteen AI_016 identities: Goldfish, Corrupt Goldfish, Piranha, Shark, Angler Fish, Arapaima, Blood Feeder, Crimson Goldfish, Gold Goldfish, Pupfish, Dolphin, Pufferfish and Orca. The runtime projects live player wet/LOS state plus bottom-slope, nearby-liquid/floor, straight-up clearance and exact `GetWaterLine` facts. The source-ordered state machine covers passive versus hostile targeting, collision reversal, three pursuit profiles, idle depth oscillation, dry server-RNG flops, Dolphin breach/surface states and Pufferfish inflation/localAI decay. Lighting, rotation and Dolphin sound remain presentation-only.
