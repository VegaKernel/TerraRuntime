...
/// </summary>
public sealed class SurfaceDecorationProvider : IWorldGenerationProvider
{
    public static readonly WorldGeneratorId GeneratorId = OptimizedProvider.GeneratorId;
    ...
    private static readonly WorldGenerationPassId ProgressionContentId = ProgressionContentProvider.ProgressionContentId;
    ...
    private readonly ProgressionValidationProvider baseline = new();
    ...
    private sealed class TerrainMorphologyPass : IWorldGenerationPass
    {
        ...
        public void Execute(IWorldGenerationContext context) =>
            TerrainMorphology.Apply(context);
    }
    ...
    private sealed class UndergroundMorphologyPass : IWorldGenerationPass
    {
        ...
        public void Execute(IWorldGenerationContext context) =>
            _ = UndergroundMorphology.Apply(context);
    }
    ...
    private sealed class JungleEcologyPass : IWorldGenerationPass
    {
        ...
        public void Execute(IWorldGenerationContext context) =>
            _ = JungleEcologyV2.Apply(context);
    }
    ...
    private sealed class ExplorationLootPass : IWorldGenerationPass
    {
        ...
        public void Execute(IWorldGenerationContext context) =>
            _ = ExplorationLoot1458.Apply(context);
    }
    ...
    private sealed class SurfaceShapingPass : IWorldGenerationPass
    {
        ...
        public void Execute(IWorldGenerationContext context)
        {
            ...
            for (int x = Math.Max(2, margin); x < width - Math.Max(2, margin); x++)
            {
                ...
                int y = Geometry.FindFirstActiveY(context.Workspace, x, startY, endY);
                int leftY = Geometry.FindFirstActiveY(context.Workspace, x - 1, startY, endY);
                int rightY = Geometry.FindFirstActiveY(context.Workspace, x + 1, startY, endY);
                ...
                if (shape != 0 && Geometry.TrySetShape(context.Workspace, x, y, shape))
                    shaped++;
            }
    ...
    private sealed class SurfaceLifePass : IWorldGenerationPass
    {
            ...
            int target)
        {
            ...
            {
                throw new InvalidOperationException(
                    "Optimized source-backed tree growth requires Workspace.");
            }
            ...
            var treeRandom = new TreeRandomAdapter1458(context.Random);
            ...
            bool TryGrowAt(int x)
            {
                ...
                if (!TreeGrower1458.TryGrow(runtimeWorkspace.TileStore, x, floor, treeRandom))
                    return false;
        ...
        private static int FindSurfaceFloor(IWorldGenerationWorkspace workspace, int x, WorldGenerationLayers layers)
        {
            ...
            return Geometry.FindFirstActiveY(workspace, x, start, end);
        }

        private static bool IsClearRectangle(IWorldGenerationWorkspace workspace, int left, int top, int width, int height)
        {
            return Geometry.IsClearRectangle(workspace, left, top, width, height);
        }
