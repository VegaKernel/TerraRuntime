...
/// </summary>
internal static class ExplorationLoot1458
{
    ...
    internal static ExplorationLootReport Apply(IWorldGenerationContext context)
    {
        ArgumentNullException.ThrowIfNull(context);
        if (context.Workspace is not Workspace workspace)
            throw new InvalidOperationException("Optimized exploration loot requires Workspace.");
        ...
        ExplorationLootReport report = new(
            sky,
            surface,
            underground,
            cavern,
    ...
    private static void RestoreCells(Workspace workspace, int left, int top, WorldGenerationTile[] old)
    {
        int index = 0;
        for (int dy = 0; dy < 2; dy++)
        for (int dx = 0; dx < 2; dx++)
...
internal readonly record struct ExplorationLootReport(
    int SkyCaches,
    int SurfaceCaches,
