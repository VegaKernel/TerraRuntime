...
/// </summary>
public sealed class LandmarkProvider : IWorldGenerationProvider
{
    ...
    private readonly PlayableProvider baseline = new();
    ...
    private sealed class LandmarkValidationPass(LandmarkState state) : IWorldGenerationPass
    {
        public void Execute(IWorldGenerationContext context)
        {
            ...
            if (context.Workspace is not Workspace runtimeWorkspace)
                throw new InvalidOperationException("Optimized landmark validation requires Workspace.");
    ...
    private static bool TryBuildPyramid(IWorldGenerationContext context, IWorldGenerationChestWorkspace chests, HorizontalSpan desert, int centerX, int surfaceY, int ordinal)
    {
        ...
        for (int row = 0; row < height; row++)
        {
            ...
            Geometry.FillSolidHorizontal(context.Workspace, centerX - half, centerX + half, y, SandstoneBrick, wall: 0);
        }
