...
/// </summary>
public sealed class SourceBackedPipeline1458 : IWorldGenerationProvider
{
    ...
    private readonly SourceBackedProvider1458 baseline = new();
    ...
    public void BuildPlan(in WorldGenerationRequest request, IWorldGenerationPlanBuilder builder)
    {
        ...
        if ((!profile.IsDefault && !pureRemix) ||
            !TerrainPass1458.IsCanonicalWorldSize(request.WidthTiles, request.HeightTiles))
        {
            capture.Replay(builder);
            return;
        }

        CapturedPass reset = capture.Require(SourceBackedProvider1458.ResetPassId);
        CapturedPass terrain = capture.Require(SourceBackedProvider1458.TerrainPassId);
        CapturedPass biomes = capture.Require(CompatibilityBiomesId);
        CapturedPass metadata = capture.Require(SourceBackedProvider1458.MetadataPassId);
        var state = new EarlyState1458();

        builder.Add(reset.Descriptor, reset.Pass);
        builder.Add(terrain.Descriptor, terrain.Pass);
        Add(builder, TerrainLayersId, WorldGenerationRngMode.VanillaSharedRng, SourceBackedProvider1458.TerrainPassId,
            new EarlyPass1458(EarlyStage1458.TerrainLayers, state, metadata.Pass));
        Add(builder, DunesId, WorldGenerationRngMode.VanillaSharedRng, TerrainLayersId,
            new EarlyPass1458(EarlyStage1458.Dunes, state));
    ...
    private static void ReplayCompatibilityTail(CapturePlanBuilder capture, IWorldGenerationPlanBuilder builder)
    {
        ...
        {
            if (entry.Descriptor.Id == SourceBackedProvider1458.ResetPassId ||
                entry.Descriptor.Id == SourceBackedProvider1458.TerrainPassId ||
                entry.Descriptor.Id == CompatibilityBiomesId ||
                entry.Descriptor.Id == SourceBackedProvider1458.MetadataPassId)
            {
                continue;
            }
...
internal enum EarlyStage1458 : byte
{
    TerrainLayers,
    Dunes,
    OceanSand,
...
internal sealed class EarlyState1458
{
    ...
    public DungeonPalette1458 DungeonPalette { get; set; }
    public List<int> MountainCaveX { get; } = [];
}

internal readonly record struct DungeonPalette1458(
    int Color,
...
internal sealed class EarlyPass1458 : IWorldGenerationPass
{
    ...
    private readonly EarlyStage1458 stage;
    private readonly EarlyState1458 state;
    private readonly IWorldGenerationPass? bootstrapPublisher;

    public EarlyPass1458(
        EarlyStage1458 stage,
        EarlyState1458 state,
    ...
    public void Execute(IWorldGenerationContext context)
    {
        ...
        Workspace workspace = context.Workspace as Workspace ??
            throw new InvalidOperationException("Source-backed early Terraria generation requires Workspace.");
        var grid = new RuntimeGrid(workspace);

        switch (stage)
        {
            case EarlyStage1458.TerrainLayers:
                CaptureTerrainState(context, workspace, grid);
                break;
            case EarlyStage1458.Dunes:
                ApplyDunes(context, workspace, grid, RequireVanilla(context));
                break;
            case EarlyStage1458.OceanSand:
                ApplyOceanSand(context, workspace, grid, RequireVanilla(context));
                break;
            case EarlyStage1458.SandPatches:
    ...
    private void CaptureTerrainState(IWorldGenerationContext context, Workspace workspace, RuntimeGrid grid)
    {
        ...
        IRandom random)
    {
        ...
        WorldGenerationRequest request = context.Request;
        VanillaWorldSeedProfile1458 profile = WorldSeedResolver1458.Resolve(in request);
        DungeonSetupProfile1458 dungeonSetup = SetupDungeonProfile(
            random,
            profile.Special == VanillaSpecialWorldSeed1458.Remix,
            b.EffectiveCrimson);
    ...
    internal static DungeonPalette1458 SetupDungeonPalette(IRandom random, bool isRemix, bool crimson)
    {
        ArgumentNullException.ThrowIfNull(random);
        ...
        return color switch
        {
            ...
            _ => new DungeonPalette1458(2, 44, 9, 483, 90, 98, 7, 1384)
        };
    }

    internal static DungeonSetupProfile1458 SetupDungeonProfile(
        IRandom random,
        bool isRemix,
        bool crimson)
    {
        DungeonPalette1458 palette = SetupDungeonPalette(random, isRemix, crimson);
        DungeonEntranceKind1458 entrance = DungeonEntranceKind1458.Legacy;
        if (random.Next(3) == 0)
            entrance = DungeonEntranceKind1458.Dome;
        if (random.Next(3) == 0)
            entrance = DungeonEntranceKind1458.Tower;

        return new DungeonSetupProfile1458(palette, entrance, random.Next());
    }
...
internal sealed class ResidualCompatibilityBiomesPass1458 : IWorldGenerationPass
{
    private readonly IWorldGenerationPass inner;
    public ResidualCompatibilityBiomesPass1458(IWorldGenerationPass inner) => this.inner = inner;
