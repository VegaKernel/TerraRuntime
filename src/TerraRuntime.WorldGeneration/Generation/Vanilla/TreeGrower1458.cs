...
/// </summary>
internal static class TreeGrower1458
{
        ...
        IWorldGenerationVanillaRandom random)
    {
        ...
        while (groundY < height &&
               ...
               TreeGrowthCatalog1458.IsCommonSapling(tiles[Index(stride, x, groundY)].TileType))
            groundY++;
        ...
        for (int y = groundY - treeHeight; y < groundY; y++)
        {
            ...
            var feature = (TreeSegmentFeature1458)random.Next(SegmentFeatureCount);
            if (y == groundY - 1 || y == groundY - treeHeight)
                feature = TreeSegmentFeature1458.Straight;

            while ((HasLeftBranch(feature) && previousLeftBranch) ||
                   (HasRightBranch(feature) && previousRightBranch))
            {
                feature = (TreeSegmentFeature1458)random.Next(SegmentFeatureCount);
            }
            ...
            SetTreeTile(ref trunk, color, coating, TreeFrameCatalog1458.Trunk(feature, variant));
            ...
            if (previousLeftBranch)
            {
                ...
                SetTreeTile(ref branch, color, coating, TreeFrameCatalog1458.LeftBranch(leafy, variant));
            }
            ...
            if (previousRightBranch)
            {
                ...
                bool leafy = random.Next(LeafyBranchRollExclusive) < LeafyBranchSuccessCount;
                ref WorldTile branch = ref tiles[Index(stride, x + 1, y)];
                SetTreeTile(ref branch, color, coating, TreeFrameCatalog1458.RightBranch(leafy, variant));
            }
        }

        var rootShape = (TreeRootShape1458)random.Next(RootShapeRollExclusive);
        ...
        if (rootShape is TreeRootShape1458.Both or TreeRootShape1458.Right)
        {
            int variant = random.Next(SegmentVariantCount);
            ref WorldTile rightRoot = ref tiles[Index(stride, x + 1, groundY - 1)];
            SetTreeTile(ref rightRoot, color, coating, TreeFrameCatalog1458.RightRoot(variant));
        }

        if (rootShape is TreeRootShape1458.Both or TreeRootShape1458.Left)
        {
            int variant = random.Next(SegmentVariantCount);
            ref WorldTile leftRoot = ref tiles[Index(stride, x - 1, groundY - 1)];
            SetTreeTile(ref leftRoot, color, coating, TreeFrameCatalog1458.LeftRoot(variant));
        }

        int baseVariant = random.Next(SegmentVariantCount);
        ref WorldTile trunkBase = ref tiles[Index(stride, x, groundY - 1)];
        if (TreeFrameCatalog1458.TryGetTrunkBase(rootShape, baseVariant, out TreeFrame1458 baseFrame))
    ...
    private static bool HasLeftBranch(TreeSegmentFeature1458 feature) =>
        feature is TreeSegmentFeature1458.LeftBranch or TreeSegmentFeature1458.BothBranches;

    private static bool HasRightBranch(TreeSegmentFeature1458 feature) =>
    ...
    private static void NormalizeRootShape(
        ref TreeRootShape1458 rootShape,
        bool hasLeftGround,
        bool hasRightGround)
    {
        if (!hasLeftGround)
        {
            if (rootShape == TreeRootShape1458.Both)
                rootShape = TreeRootShape1458.Left;
            else if (rootShape == TreeRootShape1458.Right)
                rootShape = TreeRootShape1458.None;
        }

        if (!hasRightGround)
        {
            if (rootShape == TreeRootShape1458.Both)
                rootShape = TreeRootShape1458.Right;
            else if (rootShape == TreeRootShape1458.Left)
                rootShape = TreeRootShape1458.None;
        }
    ...
    private static bool IsTreeGround(in WorldTile tile) =>
        tile.IsActive && TreeGrowthCatalog1458.IsTreeGround(tile.TileType);
    ...
    private static void SetTreeFrame(ref WorldTile tile, TreeFrame1458 frame)
    {
