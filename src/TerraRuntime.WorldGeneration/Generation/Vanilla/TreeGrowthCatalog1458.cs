...
/// </summary>
internal static class TreeGrowthCatalog1458
{
...
internal enum TreeSegmentFeature1458 : byte
{
    Straight = 0,
    TrunkVariant1 = 1,
    TrunkVariant2 = 2,
...
internal enum TreeRootShape1458 : byte
{
    Both = 0,
    Right = 1,
    Left = 2,
...
internal readonly record struct TreeFrame1458(short X, short Y);
...
/// </summary>
internal static class TreeFrameCatalog1458
{
    ...
    public static TreeFrame1458 Trunk(TreeSegmentFeature1458 feature, int variant) => feature switch
    {
        TreeSegmentFeature1458.TrunkVariant1 => Frame(column: 0, TrunkAccentRow + variant),
        TreeSegmentFeature1458.TrunkVariant2 => Frame(column: 1, variant),
        TreeSegmentFeature1458.TrunkVariant3 => Frame(column: 2, TrunkAccentRow + variant),
        TreeSegmentFeature1458.TrunkVariant4 => Frame(column: 1, TrunkAccentRow + variant),
        TreeSegmentFeature1458.LeftBranch => Frame(column: 4, variant),
        TreeSegmentFeature1458.RightBranch => Frame(column: 3, TrunkAccentRow + variant),
        TreeSegmentFeature1458.BothBranches => Frame(column: 5, TrunkAccentRow + variant),
        _ => Frame(column: 0, variant)
    };

    public static TreeFrame1458 LeftBranch(bool leafy, int variant) =>
        leafy ? Frame(column: 2, FoliageRow + variant) : Frame(column: 3, variant);

    public static TreeFrame1458 RightBranch(bool leafy, int variant) =>
        leafy ? Frame(column: 3, FoliageRow + variant) : Frame(column: 4, TrunkAccentRow + variant);

    public static TreeFrame1458 RightRoot(int variant) => Frame(column: 1, RootRow + variant);
    ...
    public static bool TryGetTrunkBase(
        ...
        int variant,
        out TreeFrame1458 frame)
    {
        frame = shape switch
        {
            TreeRootShape1458.Both => Frame(column: 4, RootRow + variant),
            TreeRootShape1458.Right => Frame(column: 0, RootRow + variant),
            TreeRootShape1458.Left => Frame(column: 3, RootRow + variant),
            _ => default
        };
        return shape != TreeRootShape1458.None;
    }

    public static TreeFrame1458 Top(bool leafy, int variant) =>
        Frame(leafy ? 1 : 0, FoliageRow + variant);

    private static TreeFrame1458 Frame(int column, int row) =>
