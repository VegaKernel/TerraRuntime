...
/// </summary>
public enum VanillaWorldValidationStatus : byte
{
    Valid = 0,
    InvalidDimensions = 1,
    MissingMetadata = 2,
...
public readonly record struct VanillaWorldValidationResult(
    VanillaWorldValidationStatus Status,
    string? Detail = null)
{
    public bool IsValid => Status == VanillaWorldValidationStatus.Valid;
}

public static class VanillaWorldGenerationValidator1458
{
        ...
        RuntimeWorldGenerationMetadataSnapshot metadata)
    {
        ...
        long activeCount = 0;
        bool isCanonical = VanillaTerrainPass1458.IsCanonicalWorldSize(width, height) && metadata.VanillaSeedProfile.IsDefault;
        ...
        foreach (WorldChest chest in chests)
        {
            ...
            if (anchor.Reserved != 0)
                return new(VanillaWorldValidationStatus.ReservedNonZero, $"Chest anchor reserved non-zero at ({chest.X},{chest.Y}).");
        }
        ...
        for (int y = 0; y < height; y++)
        {
            for (int x = 0; x < width; x++)
            {
                ...
                if (tile.LiquidAmount > 0 && tile.IsActive && IsSolidBlockingLiquid(tile.Type))
                {
                    ...
                    {
                        return new(VanillaWorldValidationStatus.InvalidLiquid, $"Solid tile type {tile.Type} with liquid {tile.LiquidAmount} at ({x},{y}).");
                    }
                }
                ...
                else
                {
                    if (tile.FrameX != 0 || tile.FrameY != 0 || tile.Shape != 0)
                        return new(VanillaWorldValidationStatus.InvalidFlags, $"Inactive tile at ({x},{y}) has non-zero frame/shape {tile.FrameX},{tile.FrameY} shape {tile.Shape}.");
                }
            }
        }

        if (isCanonical && activeCount < (long)width * 10)
            return new(VanillaWorldValidationStatus.BiomeMissing, $"Active tiles {activeCount} too sparse for {width}x{height} world.");

        // Canonical biome presence for source-backed worlds only
        if (isCanonical)
        {
            if (!foundSnow)
                return new(VanillaWorldValidationStatus.BiomeMissing, "Snow biome missing entirely (no snow/ice/slush).");
            if (!foundJungle)
                return new(VanillaWorldValidationStatus.BiomeMissing, "Jungle biome missing (no mud/jungleGrass).");
            if (!foundDesert)
                return new(VanillaWorldValidationStatus.BiomeMissing, "Desert biome missing (no sand/hardenedSand/sandstone).");
            if (!foundDungeonTile)
                return new(VanillaWorldValidationStatus.DungeonMissing, "Dungeon tiles missing (no dungeon brick).");
            if (!foundTempleTile)
                return new(VanillaWorldValidationStatus.TempleMissing, "Jungle Temple missing (no lihzahrd brick).");
            if (!foundHellstone)
                return new(VanillaWorldValidationStatus.BiomeMissing, "Underworld hellstone missing.");
        }
        ...
        // Spawn must be on solid ground or just above
        {
            ...
            if (!isCanonical && metadata.VanillaSeedProfile.IsDefault)
            {
                ...
                if (spawnTile.IsActive && IsSolidForSpawn(spawnTile.Type))
                    return new(VanillaWorldValidationStatus.InvalidSpawn, $"Spawn tile itself is solid type {spawnTile.Type} at ({sx},{sy}).");
            }
            else if (!spawnValid)
            {
                return new(VanillaWorldValidationStatus.InvalidSpawn, $"Spawn at ({sx},{sy}) has no solid ground within 3 tiles below.");
            }
        }
