...
/// </summary>
public sealed class SourceBackedProvider1458 : IWorldGenerationProvider
{
    ...
    private readonly Provider1458 compatibility = new();
    ...
    private sealed class OverlayPlanBuilder : IWorldGenerationPlanBuilder
    {
        ...
        private readonly ParityState1458 state;

        public OverlayPlanBuilder(
            IWorldGenerationPlanBuilder inner,
            ParityState1458 state)
        {
            this.inner = inner;
            this.state = state;
        }
        ...
        public void Add(WorldGenerationPassDescriptor descriptor, IWorldGenerationPass pass)
        {
            ...
            {
                inner.Add(RequireAfter(descriptor, ResetPassId), new TerrainPass1458(pass, state));
                return;
            }

            if (descriptor.Id == MetadataPassId)
            {
                inner.Add(descriptor, new MetadataParityPass1458(pass, state));
                return;
            }
...
/// RNG state and beach bounds are supplied by <see cref="BootstrapPass1458"/>.
/// </summary>
internal sealed class TerrainPass1458 : IWorldGenerationPass
{
    ...
    public TerrainPass1458(IWorldGenerationPass fallback, ParityState1458 state)
    {
    ...
    public void Execute(IWorldGenerationContext context)
    {
        ...
        VanillaWorldSeedProfile1458 seedProfile = WorldSeedResolver1458.Resolve(in request);
...
internal sealed class MetadataParityPass1458 : IWorldGenerationPass
{
    private readonly IWorldGenerationPass fallback;
    private readonly ParityState1458 state;

    public MetadataParityPass1458(IWorldGenerationPass fallback, ParityState1458 state)
    {
        this.fallback = fallback;
        this.state = state;
    }
    ...
    public void Execute(IWorldGenerationContext context)
    {
        ...
        if (state.Bootstrap is VanillaWorldGenerationBootstrapState1458 bootstrap &&
            context.Workspace is Workspace runtimeWorkspace)
        {
