...
/// the admitted ordinary bat/Slimer/Queen Slime minion roster. Harpy's source-specific wet escape and low-speed
/// wander profile are included here; its server-only projectile cadence remains an accepted post-commit effect.
/// </summary>
public static class VanillaBatMotion1458
{
        ...
        out VanillaBatPursuitResult1458 result)
    {
        ...
        if (!input.IsValid || type == VanillaNpcIds.Harpy || !VanillaBatNpcCatalog1458.TryGetDefinition(type, out _))
        {
            result = default;
            return false;
        }
        ...
        out VanillaBatMotionResult1458 result)
    {
        ...
        if (ai1 > 200f)
        {
            ...
            float wanderAcceleration = type == VanillaNpcIds.Harpy ? 0.12f : 0.2f;
            float wanderVerticalAcceleration = type == VanillaNpcIds.Harpy ? 0.07f : 0.1f;
            float wanderMaximumHorizontalSpeed = type == VanillaNpcIds.Harpy ? 3f : 4f;
            float wanderMaximumVerticalSpeed = type == VanillaNpcIds.Harpy ? 1.25f : 1.5f;
            ai2++;
            if (ai2 > 0f)
            {
