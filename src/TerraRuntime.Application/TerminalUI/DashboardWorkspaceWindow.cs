...
internal sealed class DashboardWorkspaceWindow : Runnable
{
    ...
    private void RefreshNetwork()
    {
        ...
        var lines = new List<string>(20)
        {
            ...
            $"NPC         relay {snapshot.NpcRelayedFrames:N0}  baseline {snapshot.NpcBaselineFrames:N0}  duplicate {snapshot.NpcSuppressedDuplicateFrames:N0}  cadence {snapshot.NpcSuppressedCadenceFrames:N0}  rejected {snapshot.NpcRejectedFrames:N0}  unsupported {snapshot.NpcUnsupportedCommits:N0}",
            $"Liquid      sent {snapshot.EmittedLiquidUpdates:N0}  pending {snapshot.PendingLiquidUpdates:N0}  coalesced {snapshot.CoalescedLiquidUpdates:N0}  evicted {snapshot.DroppedLiquidUpdates:N0}  unseen {snapshot.SectionFilteredLiquidUpdates:N0}",
