...
/// </summary>
internal static class VanillaOceanIntegrity1458
{
    ...
    internal static VanillaOceanIntegrityResult1458 Validate(
        ...
        double worldSurface)
    {
        ...
        int scanWidth = Math.Min(beachWidth, VanillaOceanGenerationCatalog1458.ForcedJungleOceanLength + 16);
        ...
        return new VanillaOceanIntegrityResult1458(true, null);
    }
    ...
    private static int FindSandFloor(WorldTileStore store, int x, int startY, int scanBottom)
    {
        ...
        {
            WorldTile tile = store.Get(x, y);
            if (tile.IsActive && tile.Type == VanillaOceanGenerationCatalog1458.SandTileType)
                return y;
        }
        return -1;
    }
    ...
    private static VanillaOceanIntegrityResult1458 Invalid(bool left, string detail) =>
        new(false, $"{(left ? "left" : "right")} ocean {detail}");
}

internal readonly record struct VanillaOceanIntegrityResult1458(bool IsValid, string? Detail);
