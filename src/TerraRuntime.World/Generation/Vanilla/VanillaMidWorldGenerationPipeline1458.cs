...
/// </summary>
public sealed class SourceBackedVanillaWorldGenerationMidPipeline1458 : IWorldGenerationProvider
{
    ...
    private readonly SourceBackedVanillaWorldGenerationPipeline1458 baseline = new();
    ...
    public void BuildPlan(in WorldGenerationRequest request, IWorldGenerationPlanBuilder builder)
    {
        ...
        var state = new VanillaMidWorldGenerationState1458();
        ...
        foreach (CapturedPass entry in capture.Entries)
        {
            if (entry.Descriptor.Id == CompatibilityBiomesId)
            {
                Add(builder, MudCavesToGrassId, JungleId,
                    new VanillaMidWorldGenerationPass1458(VanillaMidWorldGenerationStage1458.MudCavesToGrass, state));
                Add(builder, FullDesertId, MudCavesToGrassId,
                    new VanillaMidWorldGenerationPass1458(VanillaMidWorldGenerationStage1458.FullDesert, state));
                Add(builder, MushroomPatchesId, FullDesertId,
                    new VanillaMidWorldGenerationPass1458(VanillaMidWorldGenerationStage1458.MushroomPatches, state));
                Add(builder, MarbleId, MushroomPatchesId,
                    new VanillaMidWorldGenerationPass1458(VanillaMidWorldGenerationStage1458.Marble, state));
                Add(builder, GraniteId, MarbleId,
                    new VanillaMidWorldGenerationPass1458(VanillaMidWorldGenerationStage1458.Granite, state));
                Add(builder, FloatingIslandsId, GraniteId,
                    new VanillaMidWorldGenerationPass1458(VanillaMidWorldGenerationStage1458.FloatingIslands, state));
                Add(builder, DirtToMudId, FloatingIslandsId,
                    new VanillaMidWorldGenerationPass1458(VanillaMidWorldGenerationStage1458.DirtToMud, state));
                Add(builder, SiltId, DirtToMudId,
                    new VanillaMidWorldGenerationPass1458(VanillaMidWorldGenerationStage1458.Silt, state));
                Add(builder, ShiniesId, SiltId,
                    new VanillaMidWorldGenerationPass1458(VanillaMidWorldGenerationStage1458.Shinies, state));
                Add(builder, WebsId, ShiniesId,
                    new VanillaMidWorldGenerationPass1458(VanillaMidWorldGenerationStage1458.Webs, state));
                Add(builder, UnderworldId, WebsId,
                    new VanillaMidWorldGenerationPass1458(VanillaMidWorldGenerationStage1458.Underworld, state));
                Add(builder, CorruptionId, UnderworldId,
                    new VanillaMidWorldGenerationPass1458(VanillaMidWorldGenerationStage1458.Corruption, state));
                Add(builder, LakesId, CorruptionId,
                    new VanillaMidWorldGenerationPass1458(VanillaMidWorldGenerationStage1458.Lakes, state));
                Add(builder, SlushId, LakesId,
                    new VanillaMidWorldGenerationPass1458(VanillaMidWorldGenerationStage1458.Slush, state));

                builder.Add(
                    CloneDescriptor(residualBiomes.Descriptor, WorldGenerationRngMode.IsolatedDeterministic, [SlushId]),
                    new VanillaOceanResidualCompatibilityBiomesPass1458(residualBiomes.Pass));
                continue;
            }

            if (entry.Descriptor.Id == CompatibilityOresId)
            {
                builder.Add(entry.Descriptor, VanillaSourceBackedOreCompatibilityBarrier1458.Instance);
                continue;
            }
...
internal enum VanillaMidWorldGenerationStage1458 : byte
{
    MudCavesToGrass,
    FullDesert,
    MushroomPatches,
...
internal sealed class VanillaMidWorldGenerationState1458
{
...
internal sealed class VanillaMidWorldGenerationPass1458 : IWorldGenerationPass
{
    ...
    private readonly VanillaMidWorldGenerationStage1458 stage;
    private readonly VanillaMidWorldGenerationState1458 state;

    public VanillaMidWorldGenerationPass1458(
        VanillaMidWorldGenerationStage1458 stage,
        VanillaMidWorldGenerationState1458 state)
    {
        this.stage = stage;
        this.state = state;
    }
    ...
    public void Execute(IWorldGenerationContext context)
    {
        ...
        RuntimeWorldGenerationWorkspace workspace = context.Workspace as RuntimeWorldGenerationWorkspace ??
            throw new InvalidOperationException("Source-backed mid Terraria generation requires RuntimeWorldGenerationWorkspace.");
        ...
        switch (stage)
        {
            case VanillaMidWorldGenerationStage1458.MudCavesToGrass:
                ApplyMudCavesToGrass(context, grid, random);
                break;
            case VanillaMidWorldGenerationStage1458.FullDesert:
                ApplyFullDesert(context, grid, random);
                break;
            case VanillaMidWorldGenerationStage1458.MushroomPatches:
                ApplyMushroomPatches(context, grid, random);
                break;
            case VanillaMidWorldGenerationStage1458.Marble:
                ApplyStoneMicroBiome(context, grid, random, Marble, "marble");
                break;
            case VanillaMidWorldGenerationStage1458.Granite:
                ApplyStoneMicroBiome(context, grid, random, Granite, "granite");
                break;
            case VanillaMidWorldGenerationStage1458.FloatingIslands:
                ApplyFloatingIslands(context, grid, random);
                break;
            case VanillaMidWorldGenerationStage1458.DirtToMud:
                ApplyDirtToMud(context, grid, random);
                break;
            case VanillaMidWorldGenerationStage1458.Silt:
                ApplySilt(context, grid, random);
                break;
            case VanillaMidWorldGenerationStage1458.Shinies:
                ApplyShinies(context, grid, random);
                break;
            case VanillaMidWorldGenerationStage1458.Webs:
                ApplyWebs(context, grid, random);
                break;
            case VanillaMidWorldGenerationStage1458.Underworld:
                ApplyUnderworld(context, grid, random);
                break;
            case VanillaMidWorldGenerationStage1458.Corruption:
                ApplyEvilBiome(context, grid, random);
                break;
            case VanillaMidWorldGenerationStage1458.Lakes:
                ApplyLakes(context, grid, random);
                break;
            case VanillaMidWorldGenerationStage1458.Slush:
...
/// </summary>
internal sealed class VanillaOceanResidualCompatibilityBiomesPass1458 : IWorldGenerationPass
{
    private readonly IWorldGenerationPass inner;

    public VanillaOceanResidualCompatibilityBiomesPass1458(IWorldGenerationPass inner) =>
        this.inner = inner ?? throw new ArgumentNullException(nameof(inner));

    public void Execute(IWorldGenerationContext context)
    {
        RuntimeWorldGenerationWorkspace workspace = context.Workspace as RuntimeWorldGenerationWorkspace ??
            throw new InvalidOperationException("Ocean residual requires RuntimeWorldGenerationWorkspace.");
...
/// </summary>
internal sealed class VanillaSourceBackedOreCompatibilityBarrier1458 : IWorldGenerationPass
{
    public static VanillaSourceBackedOreCompatibilityBarrier1458 Instance { get; } = new();

    private VanillaSourceBackedOreCompatibilityBarrier1458()
    {
    }
