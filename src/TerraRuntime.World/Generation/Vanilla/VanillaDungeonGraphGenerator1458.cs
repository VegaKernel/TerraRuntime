...
internal enum VanillaDungeonEntranceKind1458 : byte
{
    Legacy,
    Dome,
    Tower,
...
internal readonly record struct VanillaDungeonSetupProfile1458(
    VanillaDungeonPalette1458 Palette,
    VanillaDungeonEntranceKind1458 EntranceKind,
    int EntranceRandomSeed)
{
    public bool PrecalculatesEntrance => EntranceKind is not VanillaDungeonEntranceKind1458.Legacy;
    public int RoughEntranceHeight => EntranceKind switch
    {
        VanillaDungeonEntranceKind1458.Dome => 55,
        VanillaDungeonEntranceKind1458.Tower => 120,
        _ => 40,
    };
}
...
/// </summary>
internal static class VanillaDungeonGenerationCatalog1458
{
...
internal readonly record struct VanillaDungeonPoint1458(int X, int Y);

internal readonly record struct VanillaDungeonBounds1458(int Left, int Top, int Right, int Bottom)
{
    public int Width => Right - Left + 1;
    public int Height => Bottom - Top + 1;
    public static VanillaDungeonBounds1458 FromPoints(VanillaDungeonPoint1458 a, VanillaDungeonPoint1458 b, int padding) =>
        new(
...
internal readonly record struct VanillaDungeonComponent1458(
    VanillaDungeonComponentKind1458 Kind,
    VanillaDungeonPoint1458 Start,
    VanillaDungeonPoint1458 End,
    VanillaDungeonBounds1458 Bounds,
    int RandomSeed);

internal sealed class VanillaDungeonGraph1458
{
    public VanillaDungeonGraph1458(
        IReadOnlyList<VanillaDungeonComponent1458> components,
        VanillaDungeonPoint1458 anchor,
    ...
    public IReadOnlyList<VanillaDungeonComponent1458> Components { get; }
    public VanillaDungeonPoint1458 Anchor { get; }
    public ushort BrickTileType { get; }
    public ushort WallType { get; }
    public int RoomCount => Components.Count(static component =>
        component.Kind is VanillaDungeonComponentKind1458.StartingRoom or VanillaDungeonComponentKind1458.Room);
    public int HallCount => Components.Count(static component => component.Kind == VanillaDungeonComponentKind1458.Hall);
    ...
    public VanillaDungeonBounds1458 Bounds => new(
...
/// </summary>
internal static class VanillaDungeonGraphGenerator1458
{
    private static readonly VanillaDungeonPoint1458[] CardinalDirections =
    [
        new(-1, 0),
        new(1, 0),
        new(0, -1),
    ...
    public static VanillaDungeonGraph1458 Generate(
        ...
        CancellationToken cancellationToken)
    {
        ...
        VanillaDungeonSetupProfile1458 setup = workspace.VanillaDungeonSetupProfile ??
        ...
        int baseSteps = workspace.WidthTiles / VanillaDungeonGenerationCatalog1458.LayoutStepDivisor;
        ...
        int roomDelay = VanillaDungeonGenerationCatalog1458.InitialRoomDelay;
        for (int step = 0; step < steps; step++)
        {
            ...
            {
                roomDelay = VanillaDungeonGenerationCatalog1458.InitialRoomDelay;
                if (sharedRandom.Next(VanillaDungeonGenerationCatalog1458.BranchChance) == 0)
                {
                    VanillaDungeonPoint1458 saved = cursor;
                    (VanillaDungeonComponent1458 hall, cursor, lastHall) = renderer.RenderHall(
        ...
        components.Add(renderer.RenderRoom(cursor, sharedRandom.Next(), startingRoom: false));
        VanillaDungeonComponent1458 topRoom = components
            .Where(static component => component.Kind is VanillaDungeonComponentKind1458.StartingRoom or VanillaDungeonComponentKind1458.Room)
            .MinBy(static component => component.Bounds.Top);
        VanillaDungeonPoint1458 entranceCursor = new((topRoom.Bounds.Left + topRoom.Bounds.Right) / 2, topRoom.Bounds.Top + 4);
        VanillaDungeonPoint1458 entranceTarget = new(entranceX, Math.Max(10, entranceSurface - 2));
        int entranceRoomDelay = VanillaDungeonGenerationCatalog1458.InitialRoomDelay;
        ...
        for (int attempt = 0; attempt < 100 && entranceCursor.Y > entranceTarget.Y; attempt++)
        {
                ...
                entranceCursor.Y > worldSurface + 100d)
            {
                ...
                VanillaDungeonPoint1458 saved = entranceCursor;
                (VanillaDungeonComponent1458 branchHall, entranceCursor, lastHall) = renderer.RenderHall(
        ...
        VanillaDungeonPoint1458 entranceEnd = entranceCursor;
        components.Add(renderer.RenderEntrance(entranceEnd, setup.EntranceKind, setup.EntranceRandomSeed));

        return new VanillaDungeonGraph1458(components, entranceEnd, setup.Palette.BrickTileType, setup.Palette.BrickWallType);
    }

    private static bool ConsumeDecorationSetup(IWorldGenerationVanillaRandom random)
    {
        ConsumeUniqueStyles(random, VanillaDungeonGenerationCatalog1458.ShelfStyleMinimum,
            VanillaDungeonGenerationCatalog1458.ShelfStyleMaximumExclusive);
        ConsumeUniqueStyles(random, 0, VanillaDungeonGenerationCatalog1458.LanternStyleCount);
    ...
    {
        Span<int> styles = stackalloc int[VanillaDungeonGenerationCatalog1458.DecorationStyleCount];
    ...
    private sealed class Renderer
    {
        ...
        public VanillaDungeonComponent1458 RenderRoom(VanillaDungeonPoint1458 origin, int seed, bool startingRoom)
        {
            var random = new DungeonUnifiedRandom1458(seed);
            int strength = VanillaDungeonGenerationCatalog1458.RoomStrengthBase +
                random.Next(VanillaDungeonGenerationCatalog1458.RoomStrengthVariation);
            ...
            VanillaDungeonPoint1458 start = new((int)x, (int)y);
            int steps = VanillaDungeonGenerationCatalog1458.RoomStepBase +
                random.Next(VanillaDungeonGenerationCatalog1458.RoomStepVariation);
            int left = start.X;
            int top = start.Y;
            int right = start.X;
            int bottom = start.Y;
            for (int step = 0; step < steps; step++)
            {
                int outer = (int)(strength * VanillaDungeonGenerationCatalog1458.RoomOuterRadiusRatio) +
                    VanillaDungeonGenerationCatalog1458.RoomOuterPadding;
                int inner = (int)(strength * VanillaDungeonGenerationCatalog1458.RoomInnerRadiusRatio);
            ...
            return new(
                startingRoom ? VanillaDungeonComponentKind1458.StartingRoom : VanillaDungeonComponentKind1458.Room,
                start,
                end,
        ...
        public (VanillaDungeonComponent1458 Component, VanillaDungeonPoint1458 Cursor, VanillaDungeonPoint1458 Direction)
            RenderHall(VanillaDungeonPoint1458 origin, VanillaDungeonPoint1458 lastDirection, int seed)
        {
            var random = new DungeonUnifiedRandom1458(seed);
            int strength = VanillaDungeonGenerationCatalog1458.HallStrengthBase +
                random.Next(VanillaDungeonGenerationCatalog1458.HallStrengthVariation);
            int steps = VanillaDungeonGenerationCatalog1458.HallStepBase +
                random.Next(VanillaDungeonGenerationCatalog1458.HallStepVariation);
            if (random.Next(VanillaDungeonGenerationCatalog1458.LargeHallChance) == 0)
            {
                strength *= 2;
                steps /= 2;
            }

            VanillaDungeonPoint1458 direction = ChooseDirection(origin, lastDirection, random);
            ...
            VanillaDungeonPoint1458 end = new((int)x, (int)y);
            VanillaDungeonBounds1458 bounds = VanillaDungeonBounds1458.FromPoints(origin, end, strength + 6);
            return (new(VanillaDungeonComponentKind1458.Hall, origin, end, bounds, seed), end, direction);
        }

        public VanillaDungeonComponent1458 RenderLegacyEntranceSegment(
            VanillaDungeonPoint1458 start,
            VanillaDungeonPoint1458 target,
            int seed,
            bool useSkewedEntranceHalls,
            out VanillaDungeonPoint1458 end)
        {
            var random = new DungeonUnifiedRandom1458(seed);
            ...
            return new(VanillaDungeonComponentKind1458.EntranceHall, start, end,
                VanillaDungeonBounds1458.FromPoints(start, end, strength + 10), seed);
        }

        public VanillaDungeonComponent1458 RenderPrecalculatedEntranceSegment(
            VanillaDungeonPoint1458 start,
            VanillaDungeonPoint1458 target,
            int requestedSteps,
            int seed,
            out VanillaDungeonPoint1458 end)
        {
            ...
            return new(VanillaDungeonComponentKind1458.EntranceHall, start, end,
                VanillaDungeonBounds1458.FromPoints(start, end, strength + 6), seed);
        }

        public VanillaDungeonComponent1458 RenderEntrance(
            VanillaDungeonPoint1458 anchor,
            VanillaDungeonEntranceKind1458 kind,
            int seed)
        {
            (int halfWidth, int height) = kind switch
            {
                VanillaDungeonEntranceKind1458.Dome => (18, 30),
                VanillaDungeonEntranceKind1458.Tower => (13, 48),
                _ => (10, 24),
            };
            ...
            return new(VanillaDungeonComponentKind1458.Entrance, new(anchor.X, top), anchor,
                new(anchor.X - halfWidth, top, anchor.X + halfWidth, anchor.Y + 7), seed);
        }

        private VanillaDungeonPoint1458 ChooseDirection(
            VanillaDungeonPoint1458 origin,
            VanillaDungeonPoint1458 lastDirection,
            DungeonUnifiedRandom1458 random)
        {
            Span<VanillaDungeonPoint1458> candidates = stackalloc VanillaDungeonPoint1458[4];
            int count = 0;
            foreach (VanillaDungeonPoint1458 direction in CardinalDirections)
            {
