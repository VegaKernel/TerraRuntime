...
public sealed class QueenSlimeLootPipelineTests
{
    [Theory]
    ...
    public void Authoritative_player_kill_delivers_mode_correct_loot_once_and_marks_progression(bool expert, bool master)
    {
        ...
        Assert.True(VanillaNpcDefinitionCatalog.TryGet(VanillaNpcIds.QueenSlime, out var definition));
        var update = new NpcStateUpdate(VanillaNpcIds.QueenSlime.Value,
            checked((short)VanillaNpcIds.QueenSlime.Value), 100, 200, 0, 0, 0, default,
        ...
        Assert.True(progression.IsCompleted(VanillaWorldProgressionId.QueenSlime));
        ...
        if (expert)
        {
            ...
            Assert.Equal((short)4957, bag.Drop.ItemNetId);
            ...
            Assert.DoesNotContain(attackerDrops, static entry => entry.Drop.ItemNetId is 4986 or 4758 or 4980 or 4981 or 4982 or 4983 or 4984);
        }
        else
        {
            Assert.DoesNotContain(attackerDrops, static entry => entry.Message == 90);
            Assert.Equal(0, leases.ActiveLeaseCount);
            var gel = Assert.Single(attackerDrops, static entry => entry.Drop.ItemNetId == 4986);
            Assert.InRange(gel.Drop.Stack, (short)25, (short)75);
            Assert.Single(attackerDrops, static entry => entry.Drop.ItemNetId is 4982 or 4983 or 4984);
        }
        Assert.Equal(master, attackerDrops.Any(static entry => entry.Drop.ItemNetId == 4950));
