...
public sealed class SourceBackedSurfaceFinish1458Tests
{
    [Fact]
    public void Canonical_ordinary_world_extends_source_order_through_grass_wall()
    {
        var provider = new SourceBackedSurfaceFinish1458();
        ...
        Assert.Contains(SourceBackedSurfaceFinish1458.GrassWallId,
            secrets.Descriptor.RequiredAfter.ToArray());

        CaptureEntry metadata = Find(builder, "terraria:1.4.5.8/Metadata");
        Assert.IsType<SpawnPreservingMetadataPass1458>(metadata.Pass);
    }
    ...
    public void Noncanonical_world_keeps_existing_compatibility_plan_unchanged()
    {
        ...
        Assert.DoesNotContain(builder.Entries, static entry =>
            entry.Descriptor.Id == SourceBackedSurfaceFinish1458.QuickCleanupId);
        Assert.DoesNotContain(builder.Entries, static entry =>
            entry.Descriptor.Id == SourceBackedSurfaceFinish1458.GrassWallId);
    }
    ...
    public void Spawn_preserving_metadata_wrapper_restores_source_spawn_after_fallback()
    {
        ...
        var pass = new SpawnPreservingMetadataPass1458(fallback, state);
        var workspace = new Workspace(100, 80);
    ...
    private sealed class TestContext(Workspace workspace) : IWorldGenerationContext
    {
