...
public sealed class VanillaTreeGrower1458Tests
{
    ...
    public void Scripted_source_path_pins_trunk_branches_roots_and_leafy_top_frames()
    {
        ...
        bool grown = VanillaTreeGrower1458.TryGrow(store, x: 10, checkedY: 30, random);
    ...
    public void Consecutive_branch_rerolls_preserve_the_shared_rng_order()
    {
        ...
        Assert.True(VanillaTreeGrower1458.TryGrow(store, 10, 30, random));
    ...
    {
        Assert.Equal(12, CountTiles(VanillaTreeGrowthCatalog1458.IsTreeGround));
        Assert.Equal(4, CountTiles(VanillaTreeGrowthCatalog1458.IsCommonSapling));
        Assert.Equal(27, CountTiles(VanillaTreeGrowthCatalog1458.IsReplaceableGrowthTile));
        Assert.Equal(27, CountWalls(VanillaTreeGrowthCatalog1458.AllowsPlantGrowth));

        Assert.True(VanillaTreeGrowthCatalog1458.IsTreeGround(VanillaTileIds.Grass));
        Assert.True(VanillaTreeGrowthCatalog1458.IsTreeGround(VanillaTileIds.JungleGrass));
        Assert.True(VanillaTreeGrowthCatalog1458.IsTreeGround(VanillaTileIds.MushroomGrass));
        Assert.True(VanillaTreeGrowthCatalog1458.IsTreeGround(VanillaTileIds.SnowBlock));
        Assert.False(VanillaTreeGrowthCatalog1458.IsTreeGround(VanillaTileIds.Dirt));
        Assert.True(VanillaTreeGrowthCatalog1458.AllowsPlantGrowth(VanillaWallIds.None));
        Assert.False(VanillaTreeGrowthCatalog1458.AllowsPlantGrowth(VanillaWallIds.Stone));
    }
    ...
    private static void AssertRejected(Action<WorldTileStore> arrange, int expectedRandomCalls)
    {
        ...
        Assert.False(VanillaTreeGrower1458.TryGrow(store, 10, 30, random));
