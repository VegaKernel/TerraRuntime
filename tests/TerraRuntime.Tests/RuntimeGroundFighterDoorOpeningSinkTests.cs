...
public sealed class RuntimeGroundFighterDoorOpeningSinkTests
{
    ...
    [Fact]
    public void Goblin_peon_door_destruction_publishes_packet17_and_materializes_door_item()
    {
        var tiles = new WorldTileStore(new WorldDimensions(40, 40));
        for (int row = 0; row < 3; row++)
        {
        ...
        MarkPlaying(replication, source, slot: 3);
        var items = new RuntimeWorldItemStore();
        var sink = new RuntimeGroundFighterDoorOpeningSink(
            tiles,
            replication,
            tallGateOccupancy: null,
            items,
            new FixedWorldItemRandom());
        ...
        Assert.True(items.TryGetActive(0, out WorldItemSnapshot dropped));
        Assert.True(dropped.TryGetItemType(out ItemTypeId dropType));
        Assert.Equal(new ItemTypeId(25), dropType);
        Assert.Equal(1, outbound.QueuedFrames);
        TerrariaFrame frame = DequeueFrame(outbound);
        Assert.Equal(
        ...
        Assert.Equal((byte)TerrariaTileManipulationAction.KillTile, state.Action);
