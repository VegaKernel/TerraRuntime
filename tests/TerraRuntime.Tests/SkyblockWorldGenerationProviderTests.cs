...
public sealed class SkyblockWorldGenerationProviderTests
{
    ...
    {
        RuntimeWorldGenerationWorkspace corruption = Assert.IsType<RuntimeWorldGenerationWorkspace>(
            Generate(seed: 0xA17A2UL, evil: WorldGenerationEvil.Corruption).Candidate);
        RuntimeWorldGenerationWorkspace crimson = Assert.IsType<RuntimeWorldGenerationWorkspace>(
    ...
    private static int CountActiveType(RuntimeWorldGenerationWorkspace candidate, int type)
    {
        int count = 0;
        for (int x = 0; x < candidate.WidthTiles; x++)
        for (int y = 0; y < candidate.HeightTiles; y++)
    ...
    private static short[] CaptureFrameX(RuntimeWorldGenerationWorkspace candidate, int type) =>
    ...
    private static short[] CaptureFrameY(RuntimeWorldGenerationWorkspace candidate, int type) =>
        CaptureProgressionStructureTiles(candidate)
            .Where(tile => tile.Type == type)
