...
public sealed class SourceBackedStartingNpc1458Tests
{
    [Fact]
    public void Canonical_ordinary_world_extends_source_order_through_guide()
    {
        var provider = new SourceBackedStartingNpc1458();
        ...
        Assert.Equal(
            SourceBackedStartingNpc1458.GuideId,
        ...
        Assert.IsType<StartingGuidePass1458>(builder.Entries[grassWall + 1].Pass);
    ...
    {
        var workspace = new Workspace(128, 96);

        Assert.True(workspace.TryAddGeneratedTownNpc(
            StartingGuidePass1458.GuideNetId,
            StartingGuidePass1458.StableGuideName,
            64 * 16f,
            40 * 16f,
            homeless: true,
            homeTileX: 64,
    ...
    public void Guide_pass_registers_town_npc_at_source_backed_spawn_without_consuming_rng()
    {
        ...
        StartingGuidePass1458.Instance.Execute(context);
    ...
    {
        public TestContext(WorldGenerationRequest request, Workspace workspace)
        {
            Request = request;
            Workspace = workspace;
            Metadata = workspace;
