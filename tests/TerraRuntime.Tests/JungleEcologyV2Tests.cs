...
public sealed class OptimizedJungleEcologyV2Tests
{
    ...
    {
        Assert.Equal(hives, JungleEcologyV2.ResolveHiveTarget(width));
        Assert.Equal(mushrooms, JungleEcologyV2.ResolveMushroomTarget(width));
    }
    ...
    public void Canonical_medium_builds_distinct_queen_bee_ready_hives_and_mushroom_pockets()
    {
        ...
        JungleEcologyV2.HiveComponent[] hives = JungleEcologyV2.CaptureHiveComponents(world);
        Assert.Equal(2, hives.Length);
        foreach (JungleEcologyV2.HiveComponent hive in hives)
        {
            JungleEcologyV2.HiveQuality quality = JungleEcologyV2.InspectHive(world, hive);
        ...
        Assert.True(larvaTiles >= ProgressionContentProvider.ResolveLarvaTarget(request.WidthTiles) * 9,
        ...
        int mushroomComponents = CountMaterialComponents(
            ...
            minimumCells: 8);
        Assert.True(mushroomComponents >= JungleEcologyV2.ResolveMushroomTarget(request.WidthTiles),
            $"Expected at least {JungleEcologyV2.ResolveMushroomTarget(request.WidthTiles)} glowing-mushroom material components, found {mushroomComponents}.");
    }

    [Fact]
    public void Algorithm_version_is_explicit() => Assert.Equal(2, JungleEcologyV2.AlgorithmVersion);

    private static int CountActiveTiles(Workspace world, ushort type)
    {
        int count = 0;
    ...
    private static int CountMaterialComponents(Workspace world, ushort type, int minimumCells)
    {
