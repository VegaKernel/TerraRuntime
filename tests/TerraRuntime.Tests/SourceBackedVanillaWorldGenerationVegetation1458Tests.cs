...
public sealed class SourceBackedVanillaWorldGenerationVegetation1458Tests
{
    [Fact]
    public void Canonical_ordinary_world_extends_source_order_through_mushrooms()
    {
        var provider = new SourceBackedVanillaWorldGenerationVegetation1458();
        ...
        Assert.Contains(SourceBackedVanillaWorldGenerationVegetation1458.MushroomsId, secrets.Descriptor.RequiredAfter.ToArray());
    }
    ...
    public void Noncanonical_world_keeps_existing_compatibility_plan_unchanged(int width, int height)
    {
        ...
        Assert.DoesNotContain(builder.Entries, static entry =>
            entry.Descriptor.Id == SourceBackedVanillaWorldGenerationVegetation1458.SunflowersId);
        Assert.DoesNotContain(builder.Entries, static entry =>
