...
public sealed class PlayerEquipmentPacket5NormalizerTests
{
    [Fact]
    ...
    {
        Assert.True(PlayerEquipmentPacket5Normalizer.TryNormalizeNetId(input, out ItemTypeId itemType));
    ...
    public void Invalid_net_id_does_not_cross_the_gameplay_boundary(short input) =>
        Assert.False(PlayerEquipmentPacket5Normalizer.TryNormalizeNetId(input, out _));
    ...
    public void Empty_or_invalid_items_become_canonical_air()
    {
        ...
        PlayerEquipmentCommitRequest normalizedEmpty = PlayerEquipmentPacket5Normalizer.Normalize(in empty);
        PlayerEquipmentCommitRequest normalizedInvalid = PlayerEquipmentPacket5Normalizer.Normalize(in invalid);
    ...
    public void Relay_preserves_favorite_and_strips_transient_or_unknown_flags()
    {
        ...
        Assert.Equal(
            (7, (byte)3, (short)3764, PlayerEquipmentCommitRequest.FavoriteItemFlag),
            Fields(normalized));
