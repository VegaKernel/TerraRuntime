...
public sealed class SourceBackedVanillaWorldGenerationMidPipeline1458Tests
{
    [Fact]
    public void Canonical_ordinary_world_expands_source_order_from_jungle_through_slush()
    {
        var provider = new SourceBackedVanillaWorldGenerationMidPipeline1458();
        ...
        Assert.IsType<VanillaOceanResidualCompatibilityBiomesPass1458>(residualBiomes.Pass);
        Assert.Contains(SourceBackedVanillaWorldGenerationMidPipeline1458.SlushId, residualBiomes.Descriptor.RequiredAfter.ToArray());
    ...
    public void Noncanonical_world_keeps_existing_compatibility_plan_unchanged()
    {
        ...
        Assert.DoesNotContain(builder.Entries, static entry =>
            entry.Descriptor.Id == SourceBackedVanillaWorldGenerationMidPipeline1458.FullDesertId);
        Assert.DoesNotContain(builder.Entries, static entry =>
            entry.Descriptor.Id == SourceBackedVanillaWorldGenerationMidPipeline1458.ShiniesId);
    }
