...
public sealed class SourceBackedProvider1458Tests
{
    [Theory]
    ...
    {
        Assert.Equal(expected, TerrainPass1458.IsCanonicalWorldSize(width, height));
    }

    [Fact]
    public void Source_backed_provider_inserts_reset_before_terrain_without_changing_generator_identity()
    {
        var provider = new SourceBackedProvider1458();
        ...
        CaptureEntry reset = Assert.Single(builder.Entries, static entry =>
            entry.Descriptor.Id == SourceBackedProvider1458.ResetPassId);
        ...
        Assert.IsType<BootstrapPass1458>(reset.Pass);
        Assert.IsType<TerrainPass1458>(terrain.Pass);
        Assert.IsType<MetadataParityPass1458>(metadata.Pass);
        Assert.Contains(
            SourceBackedProvider1458.ResetPassId,
    ...
    public void Ordinary_small_world_reset_matches_pinned_rng_checkpoint_and_world_origins()
    {
        ...
        VanillaWorldGenerationBootstrapState1458 bootstrap =
            BootstrapPass1458.Run(adapter, 4200, effectiveCrimson: false);
    ...
    public void Noncanonical_reset_is_a_true_noop_and_does_not_advance_vanilla_rng()
    {
        ...
        var pass = new BootstrapPass1458(state);
        var request = new WorldGenerationRequest(
            Provider1458.GeneratorId,
            "Synthetic",
            Seed: 1,
    ...
    public void Canonical_default_terrain_consumes_reset_state_and_publishes_layers()
    {
        ...
        var state = new ParityState1458();
        var bootstrapPass = new BootstrapPass1458(state);
        var terrainPass = new TerrainPass1458(fallback, state);
        var request = new WorldGenerationRequest(
            Provider1458.GeneratorId,
            "Canonical",
            Seed: 1458,
    ...
    public void Noncanonical_world_keeps_existing_compatibility_terrain()
    {
        bool fallbackExecuted = false;
        var fallback = new ActionPass(_ => fallbackExecuted = true);
        var pass = new TerrainPass1458(fallback, new ParityState1458());
        var request = new WorldGenerationRequest(
            Provider1458.GeneratorId,
            "Compatibility",
            Seed: 1,
