...
public sealed class OptimizedTerrainMorphologyTests
{
    ...
    public void Canonical_world_sizes_have_bounded_distinct_macro_relief()
    {
        ...
        foreach ((int width, int height, ulong seed) in fixtures)
        {
            ...
            int[] first = TerrainMorphology.BuildTargetSurfaceProfile(seed, width, height, source);
            int[] replay = TerrainMorphology.BuildTargetSurfaceProfile(seed, width, height, source);
            TerrainMorphology.ProfileMetrics metrics =
                TerrainMorphology.AnalyzeProfile(source, first);
    ...
    public void Different_seeds_change_the_morphology_fingerprint()
    {
        ...
        int[] first = TerrainMorphology.BuildTargetSurfaceProfile(1UL, width, height, source);
        int[] second = TerrainMorphology.BuildTargetSurfaceProfile(2UL, width, height, source);

        ulong firstFingerprint = TerrainMorphology.AnalyzeProfile(source, first).Fingerprint;
        ulong secondFingerprint = TerrainMorphology.AnalyzeProfile(source, second).Fingerprint;
    ...
    public void Spawn_oceans_and_evil_altar_anchor_remain_fixed()
    {
        ...
        int[] target = TerrainMorphology.BuildTargetSurfaceProfile(seed, width, height, source);
    ...
    {
        Assert.Equal(2, TerrainMorphology.AlgorithmVersion);
    }
}
