...
public sealed class SourceBackedPipeline1458Tests
{
    [Fact]
    public void Canonical_ordinary_world_expands_the_real_early_pass_order_through_jungle()
    {
        var provider = new SourceBackedPipeline1458();
    ...
    public void Noncanonical_world_keeps_the_existing_compatibility_plan_unchanged()
    {
        ...
        Assert.Contains(builder.Entries, static e => e.Descriptor.Id == SourceBackedProvider1458.MetadataPassId);
    }
    ...
    {
        Assert.Equal((minimum, maximum), EarlyPass1458.GetDuneCountRange(width));
    }
        ...
        ushort expectedBrick)
    {
        ...
        DungeonPalette1458 palette =
            EarlyPass1458.SetupDungeonPalette(random, remix, crimson);
    ...
    public void Dungeon_setup_consumes_palette_both_entrance_rolls_and_component_seed_in_source_order()
    {
        ...
        DungeonSetupProfile1458 profile =
            EarlyPass1458.SetupDungeonProfile(random, isRemix: false, crimson: false);

        Assert.Equal(4, random.CallCount);
        Assert.Equal((ushort)44, profile.Palette.BrickTileType);
        Assert.Equal(DungeonEntranceKind1458.Tower, profile.EntranceKind);
    ...
    private sealed class RecordingRandom(params int[] values) : IWorldGenerationVanillaRandom, EarlyPass1458.IRandom
    {
        private int index;
