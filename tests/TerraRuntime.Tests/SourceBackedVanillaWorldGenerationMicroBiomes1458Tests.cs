...
public sealed class SourceBackedVanillaWorldGenerationMicroBiomes1458Tests
{
    [Fact]
    public void Canonical_ordinary_world_registers_one_micro_biomes_pass_after_larva()
    {
        var provider = new SourceBackedVanillaWorldGenerationMicroBiomes1458();
        ...
        Assert.True(larva >= 0);
        Assert.Equal(SourceBackedVanillaWorldGenerationMicroBiomes1458.MicroBiomesId, builder.Entries[larva + 1].Descriptor.Id);
        Assert.Equal(WorldGenerationRngMode.VanillaSharedRng, builder.Entries[larva + 1].Descriptor.RngMode);
        Assert.Contains(SourceBackedVanillaWorldGenerationUndergroundFinish1458.LarvaId,
            builder.Entries[larva + 1].Descriptor.RequiredAfter.ToArray());

        CaptureEntry secrets = Find(builder, "terraria:1.4.5.8/SecretSeeds");
        Assert.Contains(SourceBackedVanillaWorldGenerationMicroBiomes1458.MicroBiomesId,
    ...
    {
        Assert.Equal((ushort)162, VanillaMicroBiomesWorldGenerationPass1458.ThinIce);
        Assert.Equal((ushort)141, VanillaMicroBiomesWorldGenerationPass1458.Explosives);
        Assert.Equal((ushort)215, VanillaMicroBiomesWorldGenerationPass1458.Campfire);
        Assert.Equal((ushort)314, VanillaMicroBiomesWorldGenerationPass1458.MinecartTrack);
        Assert.True(VanillaWorldFrameImportance326.IsFrameImportant(VanillaMicroBiomesWorldGenerationPass1458.Campfire));
        Assert.True(VanillaWorldFrameImportance326.IsFrameImportant(VanillaMicroBiomesWorldGenerationPass1458.LargePiles2));
    }

    [Fact]
    public void Track_placer_refuses_to_cut_through_frame_important_object()
    {
        var workspace = new RuntimeWorldGenerationWorkspace(128, 256);
        workspace.TileStore.SetInitialPopulationTile(30, 30, new WorldTile
        {
            Type = VanillaMicroBiomesWorldGenerationPass1458.Campfire,
            Flags = WorldTileFlags.Active,
            FrameX = 0,
            FrameY = 0
        });
        var grid = new VanillaMicroBiomesWorldGenerationPass1458.RuntimeGrid(workspace);
        var protectedAreas = new VanillaMicroBiomesWorldGenerationPass1458.ProtectedAreaIndex(workspace);

        bool placed = VanillaMicroBiomesWorldGenerationPass1458.TryPlaceTrack(
            grid,
            protectedAreas,
            new ZeroSlopeRandom(),
            startX: 10,
        ...
        Assert.False(placed);
        Assert.Equal(VanillaMicroBiomesWorldGenerationPass1458.Campfire, workspace.TileStore.Get(30, 30).Type);
        Assert.NotEqual(VanillaMicroBiomesWorldGenerationPass1458.MinecartTrack, workspace.TileStore.Get(10, 30).Type);
    }
