...
public sealed class OptimizedUndergroundMorphologyTests
{
    ...
    public void Canonical_world_sizes_have_scaled_multi_family_underground_plans()
    {
        ...
        foreach ((int width, int height, ulong seed) in fixtures)
        {
            ...
            OptimizedUndergroundMorphology.PlanMetrics first =
                OptimizedUndergroundMorphology.AnalyzePlan(seed, width, height, surface, rock, underworld, ocean);
            OptimizedUndergroundMorphology.PlanMetrics replay =
                OptimizedUndergroundMorphology.AnalyzePlan(seed, width, height, surface, rock, underworld, ocean);
    ...
    public void Different_seeds_change_underground_plan_fingerprint()
    {
        ...
        ulong first = OptimizedUndergroundMorphology.AnalyzePlan(1UL, width, height, surface, rock, underworld, ocean).Fingerprint;
        ulong second = OptimizedUndergroundMorphology.AnalyzePlan(2UL, width, height, surface, rock, underworld, ocean).Fingerprint;
    ...
    public void Final_optimized_plan_orders_surface_and_underground_morphology_before_walkers()
    {
        ...
        new OptimizedSurfaceDecorationWorldGenerationProvider().BuildPlan(in request, capture);
    ...
    {
        Assert.Equal(2, OptimizedUndergroundMorphology.AlgorithmVersion);
    }
