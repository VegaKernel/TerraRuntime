...
public sealed class VanillaWorldTileMutationServiceTests
{
    ...
    {
        var tiles = new WorldTileStore(new WorldDimensions(100, 100));
        Assert.True(VanillaDirtPlacement.TryPlaceOnEmpty(tiles, 20, 20));
        Assert.True(VanillaDirtPlacement.CanKillIsolated(tiles, 20, 20));
        Assert.True(VanillaDirtPlacement.TryKillIsolatedWithoutDrop(tiles, 20, 20));
        Assert.Equal(default, tiles.Get(20, 20));

        var wallOnly = new WorldTile { Wall = checked((ushort)VanillaWallIds.Stone.Value) };
        tiles.Set(30, 30, in wallOnly);
        Assert.False(VanillaDirtPlacement.TryPlaceOnEmpty(tiles, 30, 30));
    }
