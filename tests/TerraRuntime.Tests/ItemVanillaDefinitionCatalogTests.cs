...
public sealed class ItemVanillaDefinitionCatalogTests
{
    [Fact]
    public void Dirt_block_exposes_only_verified_placement_defaults()
    {
        Assert.True(VanillaDefinitionCatalog.TryGet(
        ...
        VanillaItemPrefixFamily prefixFamily)
    {
        ...
        Assert.True(VanillaDefinitionCatalog.TryGetWorldDrop(
            type,
        ...
        Assert.True(VanillaDefinitionCatalog.TryGetRuntimeDefaults(type, out VanillaItemRuntimeDefaults runtime));
    ...
    {
        Assert.True(VanillaDefinitionCatalog.IsValidKnownStack(VanillaItemIds.DirtBlock, 9_999));
        Assert.False(VanillaDefinitionCatalog.IsValidKnownStack(VanillaItemIds.DirtBlock, 10_000));
        Assert.True(VanillaDefinitionCatalog.IsValidKnownStack(new ItemTypeId(1), short.MaxValue));
        Assert.False(VanillaDefinitionCatalog.IsValidKnownStack(new ItemTypeId(1), 0));
    }

    [Fact]
    public void Slime_staff_exposes_verified_use_timing_without_claiming_an_item_use_capability()
    {
        Assert.True(VanillaDefinitionCatalog.TryGetUseTiming(
    ...
    {
        Assert.False(VanillaDefinitionCatalog.TryGetPickTool(
            VanillaItemIds.DirtBlock,
            out _));
        Assert.False(VanillaDefinitionCatalog.TryGetPlacement(
            VanillaItemIds.CopperPickaxe,
            out _));
    ...
    {
        Assert.True(VanillaDefinitionCatalog.TryGetPlacement(
            VanillaItemIds.DirtBlock,
            out VanillaItemPlacementDefinition placement));
        Assert.Equal(VanillaTileIds.Dirt, placement.TileType);
        Assert.True(placement.Consumable);

        Assert.True(VanillaDefinitionCatalog.TryGetPickTool(
            VanillaItemIds.CopperPickaxe,
            out VanillaItemPickToolDefinition pickTool));
        Assert.Equal(VanillaDefinitionCatalog.CopperPickaxePickPower, pickTool.PickPower);
        Assert.Equal(VanillaDefinitionCatalog.CopperPickaxeTileBoost, pickTool.TileBoost);
    }
}
