...
public sealed class LateHardmodeBossParityTests
{
    ...
    [Fact]
    public void Empress_lance_ring_uses_source_classic_direction_normalization()
    {
        var stepper = CreateStepper(dayTime: false);
        NpcSnapshot empress = CreateNpc(VanillaNpcIds.EmpressOfLight, new NpcAiState(4f, 0f, 0f, 0f), life: 70_000);
    ...
    public void Empress_sun_dance_count_uses_expert_or_rage_not_phase_two(bool expertMode, float phase, int expectedCount)
    {
        var stepper = CreateStepper(dayTime: false);
        stepper.SetWorldConditions(dayTime: false, slimeRainActive: false, expertMode: expertMode);
        NpcSnapshot empress = CreateNpc(VanillaNpcIds.EmpressOfLight, new NpcAiState(6f, 0f, 0f, phase), life: 70_000);
        NpcStateUpdate proposed = Proposed(in empress, empress.Ai);
        Span<NpcAiProjectileIntent> intents = stackalloc NpcAiProjectileIntent[8];
    ...
    {
        var stepper = CreateStepper(dayTime: false);
        NpcSnapshot empress = CreateNpc(VanillaNpcIds.EmpressOfLight, new NpcAiState(2f, 0f, 0f, 0f), life: 70_000);
        NpcStateUpdate proposed = Proposed(in empress, empress.Ai);
        Span<NpcAiProjectileIntent> intents = stackalloc NpcAiProjectileIntent[1];

        Assert.Equal(1, stepper.PlanProjectileSpawns(in empress, in proposed, intents));
        Assert.Equal(-6f, intents[0].VelocityX, precision: 3);
        Assert.Equal(0f, intents[0].VelocityY, precision: 3);
